...
/* ─────────────────────────────── tokens ─────────────────────────── */
const T = {
  bg:"#040404", surface:"#0a0a0a", surface2:"#0f0f0f",
  border:"rgba(255,255,255,0.07)", borderHover:"rgba(255,255,255,0.14)",
  amber:"#f59e0b", amberDim:"rgba(245,158,11,0.1)", amberBorder:"rgba(245,158,11,0.3)",
  green:"#4ade80", greenDim:"rgba(74,222,128,0.1)", greenBorder:"rgba(74,222,128,0.2)",
  red:"#f87171",   redDim:"rgba(248,113,113,0.08)",  redBorder:"rgba(248,113,113,0.2)",
  purple:"#a78bfa",
  muted:"#a1a1aa", dim:"#71717a", deep:"#52525b",
};
...
const fmtFull  = d => new Date(d+"T00:00:00").toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric"});
const fmtShort = d => new Date(d+"T00:00:00").toLocaleDateString("en-US",{month:"short",day:"numeric"});
const fmtMY    = (y,m) => new Date(y,m,1).toLocaleDateString("en-US",{month:"long",year:"numeric"});
function to24Hour(t){const[time,mod]=t.split(" ");let[h,m]=time.split(":");if(h==="12")h="00";if(mod==="PM")h=String(parseInt(h)+12);return`${h.padStart(2,"0")}:${m}:00`;}

const DAYS  = ["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"];
const HOURS = Array.from({length:19},(_,i)=>{const h=Math.floor(i/2)+9;const m=i%2===0?"00":"30";return`${h.toString().padStart(2,"0")}:${m}`;}).filter(t=>t<="18:00");

const STATUS_CFG = {
  pending_shop:{ label:"Awaiting Arrival", color:"#fb923c", bg:"rgba(251,146,60,0.08)",  border:"rgba(251,146,60,0.2)" },
  ...
  cancelled:   { label:"Cancelled",        color:T.dim,     bg:"rgba(82,82,91,0.05)",    border:"rgba(82,82,91,0.1)" },
};
...
function Toast({toast}){
  ...
  return(
    <div style={{position:"fixed",bottom:28,left:"50%",transform:"translateX(-50%)",zIndex:9999,padding:"11px 22px",background:ok?T.greenDim:T.redDim,border:`1px solid ${ok?T.greenBorder:T.redBorder}`,backdropFilter:"blur(20px)",display:"flex",alignItems:"center",gap:10,animation:"toastIn 0.3s ease",whiteSpace:"nowrap"}}>
      <div style={{width:6,height:6,borderRadius:"50%",background:ok?T.green:T.red,flexShrink:0}}/>
      <p style={{...sf,fontSize:7,color:ok?T.green:T.red,letterSpacing:"0.2em",textTransform:"uppercase",margin:0}}>{toast.msg}</p>
    </div>
  );
}
...
function ApptTicket({appt,onStatusChange,onReschedule,onCancel,onNotes,onStrike,isMobile}){
  ...
  const saveNote = async()=>{
    setSaving(true);
    try{ await API.patch(`barber/appointments/${appt.id}/`,{barber_notes:note}); onNotes&&onNotes(appt.id,note); }
    ...
    <div style={{background:T.surface,border:`1px solid ${T.border}`,overflow:"hidden",transition:"border-color 0.2s"}}
      ...
      <div style={{padding:isMobile?"14px 12px":"16px 20px",display:"flex",alignItems:"center",gap:isMobile?8:16,flexWrap:"wrap"}}>
        <div style={{width:isMobile?52:64,flexShrink:0,textAlign:"center",padding:"8px 0",background:T.surface2,border:`1px solid ${T.border}`}}>
          ...
          <p style={{...mono,fontSize:8,color:T.dim,marginTop:3}}>{fmtShort(appt.date)}</p>
        </div>
        <div style={{flex:1,minWidth:0}}>
          <p style={{...sf,fontSize:isMobile?10:12,fontWeight:700,textTransform:"uppercase",marginBottom:5,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
          ...
          <div style={{display:"flex",gap:10,marginTop:3,flexWrap:"wrap"}}>
            ...
            <span style={{...mono,fontSize:9,color:"#71717a"}}>📍 2509 W 4th St, Hattiesburg, MS</span>
          </div>
        </div>
        <div style={{display:"flex",gap:6,alignItems:"center",flexShrink:0,flexWrap:"wrap"}}>
          {appt.is_walk_in&&<span style={{...sf,fontSize:5,letterSpacing:"0.15em",padding:"3px 8px",background:T.amberDim,border:`1px solid ${T.amberBorder}`,color:T.amber}}>WALK-IN</span>}
          {hasDeposit&&<span style={{...sf,fontSize:5,letterSpacing:"0.15em",padding:"3px 8px",background:"rgba(34,197,94,0.08)",border:"1px solid rgba(34,197,94,0.25)",color:"#4ade80"}}>💰 DEPOSIT</span>}
          {isOnline&&!hasDeposit&&<span style={{...sf,fontSize:5,letterSpacing:"0.15em",padding:"3px 8px",background:T.amberDim,border:`1px solid ${T.amberBorder}`,color:T.amber}}>PAID</span>}
        ...
        <div style={{borderTop:`1px solid ${T.border}`,padding:"16px 20px",background:T.amberDim}}>
          ...
          {status==="pending_shop"&&(
            <div style={{marginBottom:16,padding:"14px 16px",background:"rgba(251,146,60,0.08)",border:"1px solid rgba(251,146,60,0.3)"}}>
              ...
              <button onClick={()=>onStatusChange&&onStatusChange(appt.id,"confirmed")}
                style={{padding:"11px 24px",background:"#fb923c",color:"black",...sf,fontSize:7,fontWeight:700,letterSpacing:"0.15em",textTransform:"uppercase",border:"none",cursor:"pointer",transition:"all 0.2s",
          ...
          <p style={{...sf,fontSize:6,letterSpacing:"0.4em",color:T.muted,textTransform:"uppercase",marginBottom:10}}>Update Status</p>
          <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:16}}>
            {Object.entries(STATUS_CFG).map(([key,cfg])=>(
              <button key={key} onClick={()=>onStatusChange&&onStatusChange(appt.id,key)}
                style={{padding:"7px 12px",...sf,fontSize:6,letterSpacing:"0.1em",textTransform:"uppercase",background:status===key?cfg.bg:"transparent",border:`1px solid ${status===key?cfg.color:T.border}`,color:status===key?cfg.color:T.muted,cursor:"pointer",transition:"all 0.2s"}}>
                {cfg.label}
              </button>
            ))}
          </div>
          ...
          {(status==="confirmed"||status==="no_show")&&(
            <div style={{marginBottom:16,padding:"12px 14px",background:"rgba(239,68,68,0.04)",border:"1px solid rgba(239,68,68,0.12)"}}>
              <p style={{...sf,fontSize:6,letterSpacing:"0.4em",color:"#ef4444",textTransform:"uppercase",marginBottom:10}}>⚡ Issue Strike</p>
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                ...
                <button onClick={()=>onStrike&&onStrike(appt.id,"late_cancel")}
                  style={{padding:"7px 14px",background:"rgba(245,158,11,0.06)",border:"1px solid rgba(245,158,11,0.2)",color:"#f59e0b",...sf,fontSize:6,letterSpacing:"0.1em",textTransform:"uppercase",cursor:"pointer",transition:"all 0.2s"}}
          ...
          <p style={{...sf,fontSize:6,letterSpacing:"0.4em",color:T.muted,textTransform:"uppercase",marginBottom:8}}>Barber Notes</p>
          <textarea value={note} onChange={e=>setNote(e.target.value)} rows={2}
            placeholder="Add notes about this appointment..."
            style={{width:"100%",background:T.bg,border:`1px solid ${T.border}`,padding:"10px 12px",color:"white",fontSize:13,...mono,outline:"none",resize:"vertical",marginBottom:10,borderRadius:0}}
            onFocus={e=>e.target.style.borderColor=T.amber}
            onBlur={e=>e.target.style.borderColor=T.border}/>
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            <button onClick={saveNote} disabled={saving}
              style={{padding:"8px 16px",background:saving?T.deep:T.amber,color:saving?T.dim:"black",...sf,fontSize:7,fontWeight:700,letterSpacing:"0.15em",textTransform:"uppercase",border:"none",cursor:saving?"not-allowed":"pointer",transition:"all 0.2s"}}>
              {saving?"Saving...":"Save Notes →"}
            </button>
            <button onClick={()=>onReschedule&&onReschedule(appt)}
              style={{padding:"8px 14px",...sf,fontSize:7,letterSpacing:"0.15em",textTransform:"uppercase",background:"transparent",border:`1px solid ${T.border}`,color:T.muted,cursor:"pointer",transition:"all 0.2s"}}
              onMouseEnter={e=>{e.currentTarget.style.borderColor=T.amber;e.currentTarget.style.color=T.amber;}}
...
function StripeConnectPanel({ barber, isMobile }) {
  ...
  useEffect(()=>{
    ...
    if(p.get("stripe")==="connected"){
      API.get("barber/stripe/status/").then(r=>setStatus(r.data)).catch(()=>{});
  ...
  if (!status) return (
    <div style={{ padding:"20px", display:"flex", alignItems:"center", gap:12 }}>
      <div style={{ width:16,height:16,border:"2px solid rgba(245,158,11,0.2)",borderTopColor:"#f59e0b",borderRadius:"50%",animation:"spin 0.8s linear infinite" }}/>
  ...
  return (
    <div style={{ marginBottom:32 }}>
      ...
      {/* Status banner */}
      <div style={{ padding:"20px 22px", background: fullySetUp ? "rgba(34,197,94,0.06)" : "rgba(245,158,11,0.06)",
        border:`1px solid ${fullySetUp?"rgba(34,197,94,0.25)":"rgba(245,158,11,0.25)"}`,
        clipPath:"polygon(0 0,calc(100% - 12px) 0,100% 12px,100% 100%,12px 100%,0 calc(100% - 12px))",
        marginBottom:16
      }}>
        <div style={{ display:"flex", alignItems:isMobile?"flex-start":"center", justifyContent:"space-between", flexWrap:"wrap", gap:16 }}>
          <div style={{ display:"flex", alignItems:"center", gap:14 }}>
            {/* Stripe logo block */}
            <div style={{ width:48, height:48, background:"#635bff", borderRadius:10, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0,
          ...
          <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
            {fullySetUp ? (
              <>
                <button onClick={openDashboard} disabled={opening}
                  style={{ padding:"10px 18px", background:"#635bff", color:"white", ...sf, fontSize:7.5, fontWeight:700, letterSpacing:"0.18em", textTransform:"uppercase", border:"none", cursor:opening?"wait":"pointer",
                    clipPath:"polygon(0 0,calc(100% - 7px) 0,100% 7px,100% 100%,7px 100%,0 calc(100% - 7px))",
                    boxShadow:"0 0 20px rgba(99,91,255,0.3)", transition:"opacity 0.2s", minHeight:"auto"
                  }}
                ...
                <button onClick={connectStripe} disabled={connecting}
                  style={{ padding:"10px 16px", background:"transparent", color:"#52525b", ...sf, fontSize:7, letterSpacing:"0.15em", textTransform:"uppercase", border:"1px solid rgba(255,255,255,0.1)", cursor:"pointer", transition:"all 0.2s", minHeight:"auto",
                    clipPath:"polygon(0 0,calc(100% - 6px) 0,100% 6px,100% 100%,6px 100%,0 calc(100% - 6px))"
                  }}
                  onMouseEnter={e=>{e.currentTarget.style.color="white";e.currentTarget.style.borderColor="rgba(255,255,255,0.3)";}}
                  onMouseLeave={e=>{e.currentTarget.style.color="#52525b";e.currentTarget.style.borderColor="rgba(255,255,255,0.1)";}}>
                  Manage
                </button>
              </>
            ) : (
      ...
      {/* How it works */}
      {!fullySetUp && (
        <div style={{ padding:"16px 18px", background:"rgba(99,91,255,0.04)", border:"1px solid rgba(99,91,255,0.12)" }}>
          <p style={{ ...mono, fontSize:8, color:"#635bff", letterSpacing:"0.4em", textTransform:"uppercase", marginBottom:10 }}>How It Works</p>
...
function CashAppTagField({ barber, onUpdate }) {
  ...
  const save = async () => {
    if (!tag.trim()) { setErr("Enter your $cashtag"); return; }
    ...
    <div>
      ...
      <div style={{ display:"flex", gap:8 }}>
        <div style={{ flex:1, position:"relative" }}>
          ...
          <input type="text" value={tag.startsWith("$")?tag.slice(1):tag} onChange={e=>{setTag(e.target.value);setSaved(false);}} placeholder="YourCashTag"
            style={{ width:"100%", background:"#0a0a0a", border:"1px solid rgba(0,212,95,0.3)", padding:"12px 12px 12px 28px", color:"white", fontSize:15, outline:"none", ...mono, transition:"border-color 0.2s" }}
            onFocus={e=>e.target.style.borderColor="#00d45f"} onBlur={e=>e.target.style.borderColor="rgba(0,212,95,0.3)"}
            onKeyDown={e=>e.key==="Enter"&&save()}/>
        </div>
        <button onClick={save} disabled={saving} style={{ padding:"12px 20px", background:saved?"#22c55e":"#00d45f", color:"black", ...sf, fontSize:7.5, fontWeight:700, letterSpacing:"0.2em", textTransform:"uppercase", border:"none", cursor:"pointer", flexShrink:0, minHeight:"auto", clipPath:"polygon(0 0,calc(100% - 6px) 0,100% 6px,100% 100%,6px 100%,0 calc(100% - 6px))", transition:"background 0.2s" }}>
...
function RescheduleModal({appt,onClose,onDone}){
  ...
  const submit=async()=>{
    if(!newDate||!newTime){setErr("Pick a date and time.");return;}
    setBusy(true);setErr("");
    try{
      await API.post(`barber/appointments/${appt.id}/reschedule/`,{new_date:newDate,new_time:to24Hour(newTime)});
  ...
  return(
    <div style={{position:"fixed",inset:0,zIndex:300,background:"rgba(4,4,4,0.95)",backdropFilter:"blur(12px)",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{width:"100%",maxWidth:440,background:T.surface,border:`1px solid ${T.amberBorder}`}}>
        {/* Header */}
        ...
        <div style={{padding:"20px 24px"}}>
          ...
          <label style={{...sf,fontSize:6,letterSpacing:"0.3em",color:T.muted,textTransform:"uppercase",display:"block",marginBottom:8}}>New Time</label>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(80px,1fr))",gap:5,marginBottom:20}}>
            {SLOTS.map(s=>(
              <button key={s} onClick={()=>setNewTime(s)}
                style={{padding:"7px 4px",...sf,fontSize:6,letterSpacing:"0.05em",textTransform:"uppercase",border:`1px solid ${newTime===s?T.amber:T.border}`,background:newTime===s?T.amberDim:"transparent",color:newTime===s?T.amber:T.muted,cursor:"pointer",transition:"all 0.15s"}}>
                {s}
              </button>
            ))}
          </div>
          <button onClick={submit} disabled={busy||!newDate||!newTime}
            style={{width:"100%",padding:"14px",...sf,fontSize:8,fontWeight:700,letterSpacing:"0.2em",textTransform:"uppercase",background:busy||!newDate||!newTime?T.deep:T.amber,color:busy||!newDate||!newTime?T.dim:"black",border:"none",cursor:busy||!newDate||!newTime?"not-allowed":"pointer",transition:"all 0.2s"}}>
...
/* ────────────────────────── month calendar ─────────────────────── */
function MonthCal({year, month, selectedDate, apptDates, onSelect, onPrev, onNext}){
  const today = todayISO();
  const sf    = {fontFamily:"'Syncopate',sans-serif"};
  const mono  = {fontFamily:"'DM Mono',monospace"};
  ...
  const apptCount = {};
  (apptDates||[]).forEach(d => { apptCount[d] = (apptCount[d]||0) + 1; });
  ...
  return(
    <div style={{background:T.surface, border:`1px solid ${T.border}`, overflow:"hidden",
      ...
      {/* Header */}
      <div style={{background:"#000", padding:"14px 16px", borderBottom:`1px solid ${T.border}`,
        display:"flex", justifyContent:"space-between", alignItems:"center"
      }}>
        <button onClick={onPrev}
          style={{width:32,height:32,background:"transparent",border:`1px solid ${T.border}`,
            color:T.muted,cursor:"pointer",fontSize:14,display:"flex",alignItems:"center",
        ...
        <div style={{textAlign:"center"}}>
          <p style={{...sf, fontSize:8, fontWeight:700, textTransform:"uppercase",
            letterSpacing:"0.15em", color:"white", margin:0
          }}>{monthLabel}</p>
        </div>
      ...
      <div style={{padding:"12px 12px 14px"}}>
        ...
        <div style={{display:"grid", gridTemplateColumns:"repeat(7,1fr)", marginBottom:6}}>
          {DAY_LABELS.map(d=>(
            <p key={d} style={{...sf, fontSize:5.5, textAlign:"center",
              color: d==="Sun"||d==="Sat" ? "#52525b" : T.dim,
              letterSpacing:"0.1em", textTransform:"uppercase", margin:0, padding:"4px 0"
            }}>{d}</p>
          ))}
        </div>
          ...
          {cells.map((d, i)=>{
            ...
            return(
              <button key={i} onClick={()=>onSelect(iso)}
                style={{
                  padding:"5px 2px 6px",
                  background: isSel ? T.amber : isToday ? T.amberDim : "transparent",
        ...
        {/* Legend */}
        <div style={{display:"flex",gap:12,marginTop:12,paddingTop:10,borderTop:`1px solid ${T.border}`,flexWrap:"wrap"}}>
          <div style={{display:"flex",alignItems:"center",gap:5}}>
            <div style={{width:8,height:8,background:T.amber,borderRadius:"50%"}}/>
            <span style={{...mono,fontSize:8,color:"#52525b"}}>Appointments</span>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:5}}>
            <div style={{width:12,height:12,background:T.amberDim,border:`1px solid ${T.amberBorder}`}}/>
            <span style={{...mono,fontSize:8,color:"#52525b"}}>Today</span>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:5}}>
            <div style={{width:12,height:12,background:T.amber}}/>
            <span style={{...mono,fontSize:8,color:"#52525b"}}>Selected</span>
          </div>
        </div>
      </div>
    </div>
...
export default function BarberDashboard(){
  ...
  const [nlForm,      setNlForm]      = useState({title:"",body:"",category:"general",emoji:"✂️",pinned:false});
  ...
  useEffect(()=>{
    const load=async()=>{
      try{
        ...
        setSchedule(Array.isArray(schedRes.data) ? schedRes.data : schedRes.data.appointments || schedRes.data.results || []);
        setServices(Array.isArray(svcRes.data)?svcRes.data:svcRes.data.results||[]);
        setAllBarbers(Array.isArray(barbersRes.data)?barbersRes.data:barbersRes.data.results||[]);
      }catch{ router.replace("/barber-login"); }
  ...
  const loadSchedule=useCallback(async(date)=>{
    ...
    try{
      ...
      setSchedule(Array.isArray(res.data)?res.data:res.data.appointments||res.data.results||[]);
  ...
  const loadAvailability=useCallback(async()=>{
    try{const r=await API.get("barber/availability/");setAvailability(r.data);}catch{}
  },[]);
  useEffect(()=>{if(activeTab==="availability")loadAvailability();},[activeTab,loadAvailability]);

  /* load time off */
  const loadTimeOff=useCallback(async()=>{
    try{const r=await API.get("barber/time-off/");setTimeOff(r.data);}catch{}
  },[]);
  ...
  const loadClients=useCallback(async()=>{
    try{const r=await API.get("barber/clients/");setClients(r.data);}catch{}
  },[]);
  ...
  useEffect(()=>{
    ...
    API.get("barber/service-prices/")
      .then(r=>{ setPricingList(r.data); setPricingEdits({}); })
      .catch(()=>{});
  },[activeTab]);

  const loadNewsletter=useCallback(async()=>{
    setNlLoading(true);
    try{const r=await API.get("newsletter/manage/");setNlPosts(r.data);}catch{}
  ...
  useEffect(()=>{
    API.get("barber/reschedules/").then(r=>{
  ...
  useEffect(()=>{
    ...
    setWiSlotsLoad(true); setWiSlots([]); setWiBooked([]); setWiTime("");
    API.get(`available-slots/?barber=${wiBarber}&date=${wiDate}`)
      .then(r=>{ setWiSlots(r.data.available_slots||[]); setWiBooked(r.data.booked_slots||[]); })
      .catch(()=>{})
      .finally(()=>setWiSlotsLoad(false));
  },[wiBarber,wiDate]);

  const handleWalkIn=async()=>{
    if(!wiName.trim()||!wiSvc||!wiTime){showToast("Name, service and time required.","error");return;}
    const targetBarber = wiBarber || barber?.id;
    if(!targetBarber){showToast("Select a barber.","error");return;}
    setWiLoading(true);
    try{
      let time24 = wiTime;
      if(wiTime.includes("AM")||wiTime.includes("PM")){
        const[t,mod]=wiTime.split(" "); let[h,m]=t.split(":");
        if(h==="12")h="00"; if(mod==="PM")h=String(parseInt(h)+12);
  ...
  const notifyWaitlist=async(id)=>{
    try{await API.patch(`barber/waitlist/${id}/`,{});setWaitlist(p=>p.map(w=>w.id===id?{...w,notified:true}:w));showToast("Notified!");}
    catch{showToast("Error.","error");}
  };

  const removeTimeOff=async(id)=>{
    try{await API.delete(`barber/time-off/${id}/`);setTimeOff(p=>p.filter(t=>t.id!==id));showToast("Removed.");}
    catch{showToast("Error.","error");}
  };
  ...
  const addTimeOff=async()=>{
    ...
    try{
      const r=await API.post("barber/time-off/",{date:newTimeOffDate,reason:newTimeOffReason});
      setTimeOff(p=>[...p,r.data]);setNewTimeOffDate("");setNewTimeOffReason("");showToast("Date blocked.");
    }catch{showToast("Error.","error");}
  ...
  const saveAvailability=async()=>{
    setSavingAvail(true);
    try{
      await API.post("barber/availability/",{day_of_week:editingDay,is_working:editWorking,start_time:editWorking?editStart+":00":"09:00:00",end_time:editWorking?editEnd+":00":"18:00:00"});
      await loadAvailability();setEditingDay(null);showToast("Hours saved.");
    }catch{showToast("Error saving.","error");}
    finally{setSavingAvail(false);}
  };

  const handleLogout=()=>{localStorage.removeItem("access");localStorage.removeItem("refresh");router.replace("/barber-login");};

  const summary = {
    today_online: schedule.filter(a=>a.payment_method==="online"&&a.status!=="cancelled").length,
    today_total:  schedule.filter(a=>a.status!=="cancelled").length,
    confirmed:    schedule.filter(a=>a.status==="confirmed").length,
    revenue:      schedule.filter(a=>a.payment_method==="online"&&a.status!=="cancelled").reduce((s,a)=>s+(parseFloat(a.service_price||0)),0).toFixed(2),
  };
  ...
  if(loading) return(
    <div style={{background:T.bg,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:24,position:"relative",overflow:"hidden"}}>
      ...
      <div style={{position:"absolute",left:0,right:0,height:1,background:"linear-gradient(to right,transparent,rgba(245,158,11,0.3),transparent)",animation:"scandown 6s linear infinite"}}/>
      <div style={{textAlign:"center",opacity:0,animation:"fadeIn 0.5s ease 0.1s forwards",position:"relative"}}>
        <p style={{fontFamily:"'Syncopate',sans-serif",fontSize:26,fontWeight:900,letterSpacing:"-0.06em",margin:0}}>HEADZ<span style={{color:T.amber,fontStyle:"italic"}}>UP</span></p>
        <p style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:T.dim,letterSpacing:"0.5em",textTransform:"uppercase",marginTop:8}}>Barber Portal</p>
      </div>
  ...
  return(
    <>
      ...
      <div style={{position:"fixed",top:"-10%",right:"-5%",width:700,height:700,background:"radial-gradient(circle,rgba(245,158,11,0.05) 0%,transparent 65%)",pointerEvents:"none",zIndex:0}}/>
      <div style={{position:"fixed",bottom:"10%",left:"-5%",width:500,height:500,background:"radial-gradient(circle,rgba(245,158,11,0.03) 0%,transparent 60%)",pointerEvents:"none",zIndex:0}}/>
      <div style={{position:"fixed",inset:0,zIndex:1,pointerEvents:"none",overflow:"hidden"}}>
        <div style={{position:"absolute",left:0,right:0,height:1,background:"linear-gradient(to right,transparent,rgba(245,158,11,0.18),transparent)",animation:"scandown 12s linear infinite"}}/>
      </div>

      <Toast toast={toast}/>
      {reschedModal&&<RescheduleModal appt={reschedModal} onClose={()=>setReschedModal(null)} onDone={()=>loadSchedule(selectedDate)}/>}

      {/* ══════════════════════════ NAV ══════════════════════════ */}
      <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:100,background:"rgba(4,4,4,0.96)",backdropFilter:"blur(24px)",borderBottom:`1px solid ${T.border}`}}>

        {/* Top bar */}
        <div style={{maxWidth:1280,margin:"0 auto",padding:isMobile?"0 14px":"0 32px",height:isMobile?56:60,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          {!isMobile&&(
            <div style={{textAlign:"center"}}>
              <p style={{...mono,fontSize:13,color:"white",letterSpacing:"0.15em"}}>{time}</p>
              <p style={{...mono,fontSize:8,color:T.dim,letterSpacing:"0.3em",marginTop:2}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:isMobile?8:14}}>
            ...
            <button onClick={handleLogout}
              ...
              onMouseEnter={e=>{e.currentTarget.style.background="rgba(248,113,113,0.22)";e.currentTarget.style.borderColor="rgba(248,113,113,0.8)";}}
            ...
            return(
              <button key={key} onClick={()=>setActiveTab(key)}
                style={{
                  ...
                  gap:isMobile?2:4,whiteSpace:"nowrap",flexShrink:0,
      ...
      <div style={{position:"relative",zIndex:10,maxWidth:1280,margin:"0 auto",padding:isMobile?"116px 14px max(40px,env(safe-area-inset-bottom))":"120px 32px 64px"}}>
        ...
        <div className="bd-enter" style={{marginBottom:28,paddingBottom:28,borderBottom:`1px solid ${T.border}`}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",flexWrap:"wrap",gap:16,marginBottom:24}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:10,opacity:0.3}}>
              <div style={{width:isMobile?20:60,height:1,background:`linear-gradient(to right,transparent,${T.amber})`}}/>
              <Scissors size={14}/>
              <div style={{width:isMobile?20:60,height:1,background:`linear-gradient(to left,transparent,${T.amber})`}}/>
            </div>
          </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:isMobile?"repeat(2,1fr)":"repeat(4,1fr)",gap:8}}>
            {[
              {label:"Today's Clients", value:barber.today_count??0,    accent:true,  icon:"✂️"},
              {label:"All Time",        value:barber.total_count??0,    accent:false, icon:"🏆"},
              {label:"Online Paid",     value:`$${barber.online_revenue||"0.00"}`, accent:false, icon:"💳"},
              {label:"Pay In Shop",     value:barber.pay_in_shop??0,   accent:false, icon:"💵"},
            ].map(({label,value,accent,icon})=>(
              <div key={label} style={{padding:isMobile?"16px 14px":"22px 18px",background:accent?T.amberDim:T.surface,border:`1px solid ${accent?T.amberBorder:T.border}`,position:"relative",overflow:"hidden",transition:"all 0.25s"}}
                onMouseEnter={e=>{e.currentTarget.style.borderColor=T.amber;e.currentTarget.style.background=T.amberDim;}}
                onMouseLeave={e=>{e.currentTarget.style.borderColor=accent?T.amberBorder:T.border;e.currentTarget.style.background=accent?T.amberDim:T.surface;}}>
                <div style={{position:"absolute",top:0,right:0,width:50,height:50,background:accent?"linear-gradient(225deg,rgba(245,158,11,0.25),transparent)":"linear-gradient(225deg,rgba(255,255,255,0.04),transparent)"}}/>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                  <p style={{...sf,fontSize:5,letterSpacing:"0.35em",color:accent?T.amber:T.muted,textTransform:"uppercase",margin:0}}>{label}</p>
                  <span style={{fontSize:14,opacity:0.4}}>{icon}</span>
                </div>
                <p style={{...sf,fontSize:isMobile?22:28,fontWeight:900,color:accent?T.amber:"white",lineHeight:1,margin:0}}>{value}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        {activeTab==="reschedules"&&(
          <div className="bd-enter">
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24,flexWrap:"wrap",gap:12}}>
              <div style={{display:"flex",alignItems:"center",gap:14}}>
                <div style={{width:4,height:28,background:"linear-gradient(to bottom,#a78bfa,#f59e0b)",flexShrink:0}}/>
                <div>
                  <p style={{...mono,fontSize:7,color:"rgba(167,139,250,0.5)",letterSpacing:"0.5em",textTransform:"uppercase",marginBottom:2}}>HEADZ UP · REQUESTS</p>
                  <p style={{...sf,fontSize:13,fontWeight:900,textTransform:"uppercase"}}>Reschedule Requests</p>
                </div>
              </div>
            ...
            {reschedLoading?(
              ...
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                ...
                {[...reschedules.filter(r=>r.status==="pending"), ...reschedules.filter(r=>r.status!=="pending")].map(rr=>{
                  const isPending = rr.status==="pending";
                  const isAccepted = rr.status==="accepted";
                  const isRejected = rr.status==="rejected";
                  const statusColor = isPending?"#f59e0b":isAccepted?"#22c55e":"#f87171";
                  const statusBg    = isPending?"rgba(245,158,11,0.08)":isAccepted?"rgba(34,197,94,0.06)":"rgba(248,113,113,0.06)";

                  return(
                    <div key={rr.id} style={{background:T.surface,border:`1px solid ${isPending?"rgba(245,158,11,0.25)":T.border}`,overflow:"hidden",
                      clipPath:"polygon(0 0,calc(100% - 12px) 0,100% 12px,100% 100%,12px 100%,0 calc(100% - 12px))"
                    }}>
                      {/* Top color bar */}
                      <div style={{height:2,background:isPending?"linear-gradient(to right,#f59e0b,#ef4444)":isAccepted?"#22c55e":"#f87171",opacity:0.7}}/>

                      <div style={{padding:"18px 20px"}}>
                        {/* Client + status row */}
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:10}}>
                          <div style={{display:"flex",alignItems:"center",gap:12}}>
                            <div style={{width:40,height:40,background:isPending?"rgba(245,158,11,0.12)":"rgba(255,255,255,0.04)",border:`1px solid ${isPending?T.amberBorder:T.border}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                          ...
                          <div style={{padding:"5px 14px",background:statusBg,border:`1px solid ${statusColor}33`}}>
                            <span style={{...sf,fontSize:6,letterSpacing:"0.25em",textTransform:"uppercase",color:statusColor}}>
                              {isPending?"⏳ Pending":isAccepted?"✓ Approved":"✕ Declined"}
                            </span>
                          </div>
                        </div>

                        {/* Time comparison card */}
                        <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr auto 1fr",gap:8,marginBottom:isPending?20:0,alignItems:"stretch"}}>
                          {/* Original time */}
                          <div style={{padding:"14px 16px",background:"rgba(255,255,255,0.02)",border:`1px solid ${T.border}`}}>
                            <p style={{...mono,fontSize:8,color:"#52525b",letterSpacing:"0.3em",textTransform:"uppercase",marginBottom:8}}>Original</p>
                            <p style={{...sf,fontSize:isMobile?10:11,fontWeight:700,color:isRejected?"#f59e0b":"#a1a1aa",marginBottom:4}}>{rr.original_date}</p>
                            <p style={{...sf,fontSize:isMobile?14:18,fontWeight:900,color:isRejected?"#f59e0b":"#71717a"}}>{rr.original_time}</p>
                          </div>

                          {/* Arrow */}
                          <div style={{display:"flex",alignItems:"center",justifyContent:"center",padding:isMobile?"4px 0":"0 8px"}}>
                            <span style={{...mono,fontSize:isMobile?16:20,color:isPending?"#f59e0b":"#3f3f46"}}>→</span>
                          </div>

                          {/* Requested time */}
                          <div style={{padding:"14px 16px",background:isPending?"rgba(245,158,11,0.04)":isAccepted?"rgba(34,197,94,0.04)":"rgba(255,255,255,0.02)",border:`1px solid ${isPending?T.amberBorder:isAccepted?"rgba(34,197,94,0.2)":T.border}`}}>
                            <p style={{...mono,fontSize:8,color:isPending?"rgba(245,158,11,0.5)":isAccepted?"rgba(34,197,94,0.5)":"#52525b",letterSpacing:"0.3em",textTransform:"uppercase",marginBottom:8}}>
                              {isAccepted?"✓ Confirmed":"Requested"}
                            </p>
                            <p style={{...sf,fontSize:isMobile?10:11,fontWeight:700,color:isPending?T.amber:isAccepted?"#22c55e":"#71717a",marginBottom:4}}>{rr.requested_date}</p>
                            <p style={{...sf,fontSize:isMobile?14:18,fontWeight:900,color:isPending?T.amber:isAccepted?"#22c55e":"#71717a"}}>{rr.requested_time}</p>
                          </div>
                        </div>
                          ...
                          <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                            ...
                            <button onClick={async()=>{
                              ...
                              try{
                                ...
                                showToast(`Reschedule declined — ${rr.client_name} has been notified.`,"error");
                              }catch(e){showToast(e.response?.data?.error||"Could not decline.","error");}
                            }} style={{flex:isMobile?1:"auto",padding:"12px 24px",background:"transparent",border:`1px solid ${T.redBorder}`,color:T.red,...sf,fontSize:7,fontWeight:700,letterSpacing:"0.2em",textTransform:"uppercase",cursor:"pointer",transition:"all 0.2s",
                              clipPath:"polygon(0 0,calc(100% - 7px) 0,100% 7px,100% 100%,7px 100%,0 calc(100% - 7px))"
                            }}
                              onMouseEnter={e=>e.currentTarget.style.background=T.redDim}
                              onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
        ...
        {activeTab==="schedule"&&(
          <div className="bd-enter">
            ...
            {/* ── TODAY'S QUICK STATS ── */}
            <div style={{display:"grid",gridTemplateColumns:`repeat(${isMobile?2:4},1fr)`,gap:8,marginBottom:20}}>
              {[
                {label:"Today",       value:schedule.length,                                                  color:T.amber},
                {label:"Confirmed",   value:schedule.filter(a=>a.status==="confirmed").length,                color:"#4ade80"},
                {label:"Completed",   value:schedule.filter(a=>a.status==="completed").length,                color:"#a78bfa"},
                {label:"No Shows",    value:schedule.filter(a=>a.status==="no_show").length,                  color:"#f87171"},
              ].map(({label,value,color})=>(
                <div key={label} style={{padding:"12px 14px",background:T.surface,border:`1px solid ${T.border}`,position:"relative",overflow:"hidden"}}>
                  <div style={{position:"absolute",top:0,left:0,right:0,height:2,background:color,opacity:0.6}}/>
                  <p style={{...sf,fontSize:5,letterSpacing:"0.35em",color,textTransform:"uppercase",marginBottom:6}}>{label}</p>
                  <p style={{...sf,fontSize:24,fontWeight:900,color,lineHeight:1}}>{value}</p>
                </div>
              ))}
            </div>

            {/* ── PAY IN SHOP TRACKER ── */}
            {schedule.filter(a=>a.payment_method==="shop"&&a.status==="confirmed").length > 0 && (
              <div style={{marginBottom:16,padding:"12px 16px",background:"rgba(245,158,11,0.04)",border:"1px solid rgba(245,158,11,0.15)"}}>
                <p style={{...sf,fontSize:6,letterSpacing:"0.4em",color:T.amber,textTransform:"uppercase",marginBottom:10}}>💵 Pay In Shop — {schedule.filter(a=>a.payment_method==="shop"&&a.status==="confirmed").length} pending</p>
            ...
            <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"280px 1fr",gap:20,alignItems:"start"}}>
              ...
              {/* Calendar */}
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                <MonthCal year={calYear} month={calMonth} selectedDate={selectedDate} apptDates={allApptDates}
                ...
                {/* Today button */}
                {selectedDate!==today&&(
                  <button onClick={()=>{setSelectedDate(today);setCalYear(new Date().getFullYear());setCalMonth(new Date().getMonth());}}
                    style={{padding:"10px",...sf,fontSize:7,letterSpacing:"0.2em",textTransform:"uppercase",background:T.amberDim,border:`1px solid ${T.amberBorder}`,color:T.amber,cursor:"pointer",transition:"all 0.2s"}}>
                    ← Back to Today
                  </button>
                )}
                {/* Send reminders */}
                <button onClick={async()=>{try{await API.post("barber/send-reminders/");showToast("Reminders sent!");}catch{showToast("Error.","error");}}}
                  style={{padding:"10px",...sf,fontSize:7,letterSpacing:"0.15em",textTransform:"uppercase",background:"transparent",border:`1px solid ${T.border}`,color:T.muted,cursor:"pointer",transition:"all 0.2s"}}
                  onMouseEnter={e=>{e.currentTarget.style.borderColor=T.amber;e.currentTarget.style.color=T.amber;}}
                  onMouseLeave={e=>{e.currentTarget.style.borderColor=T.border;e.currentTarget.style.color=T.muted;}}>
                  📧 Send Reminders
                </button>
              ...
              <div>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:10}}>
                  <div>
                    <p style={{...mono,fontSize:8,color:T.amber,letterSpacing:"0.4em",textTransform:"uppercase",marginBottom:4}}>
                      {selectedDate===today?"Today":"Selected Day"}
                    </p>
                    <h2 style={{...sf,fontSize:"clamp(0.9rem,2vw,1.2rem)",fontWeight:900,textTransform:"uppercase",letterSpacing:"-0.03em",color:selectedDate===today?T.amber:"white"}}>
                ...
                {schedLoading?(
                  <div style={{padding:"40px 0",display:"flex",justifyContent:"center"}}>
                    <div style={{width:18,height:18,border:`1.5px solid ${T.amberBorder}`,borderTopColor:T.amber,borderRadius:"50%",animation:"spin 0.8s linear infinite"}}/>
                  </div>
                ):schedule.length===0?(
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:6}}>
                    {schedule.map(appt=>{
                      ...
                      return(
                        <div key={appt.id}>
                          <ApptTicket appt={appt} isMobile={isMobile}
                            onStatusChange={handleStatusChange}
                            onStrike={issueStrike}
                            onReschedule={a=>setReschedModal(a)}
                            onCancel={handleCancel}
                            onNotes={(id,note)=>setSchedule(p=>p.map(a=>a.id===id?{...a,barber_notes:note}:a))}
                          />
                          {/* Pay-in-shop tracker — show after appointment time passes */}
                          {isShop && apptPast && appt.status!=="cancelled" && appt.status!=="no_show" && (
                            <div style={{marginTop:-1,padding:"10px 18px",background:shopStatus==="received"?"rgba(34,197,94,0.06)":"rgba(245,158,11,0.04)",border:`1px solid ${shopStatus==="received"?"rgba(34,197,94,0.2)":"rgba(245,158,11,0.15)"}`,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10}}>
                              <div style={{display:"flex",alignItems:"center",gap:10}}>
                              ...
                              {shopStatus!=="received"&&(
                                <div style={{display:"flex",gap:8}}>
                                  <button onClick={()=>setShopPayments(p=>({...p,[appt.id]:"received"}))}
                                    style={{padding:"7px 14px",background:"rgba(34,197,94,0.1)",border:"1px solid rgba(34,197,94,0.3)",color:"#4ade80",...sf,fontSize:6.5,letterSpacing:"0.15em",textTransform:"uppercase",cursor:"pointer",transition:"all 0.2s"}}
                                    onMouseEnter={e=>e.currentTarget.style.background="rgba(34,197,94,0.2)"}
                                    onMouseLeave={e=>e.currentTarget.style.background="rgba(34,197,94,0.1)"}>
                                    ✓ Got Paid
                                  </button>
                                  <button onClick={()=>setShopPayments(p=>({...p,[appt.id]:"pending"}))}
                                    style={{padding:"7px 14px",background:"transparent",border:`1px solid ${T.border}`,color:T.muted,...sf,fontSize:6.5,letterSpacing:"0.15em",textTransform:"uppercase",cursor:"pointer"}}>
                                    Not Yet
                                  </button>
                                </div>
                              )}
          ...
          <div className="bd-enter">
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24,flexWrap:"wrap",gap:12}}>
              <div style={{display:"flex",alignItems:"center",gap:14}}>
                <div style={{width:4,height:28,background:"linear-gradient(to bottom,#a78bfa,#f59e0b)",flexShrink:0}}/>
                <div>
                  <p style={{...mono,fontSize:7,color:"rgba(167,139,250,0.6)",letterSpacing:"0.5em",textTransform:"uppercase",marginBottom:2}}>HEADZ UP · REQUESTS</p>
                  <p style={{...sf,fontSize:13,fontWeight:900,textTransform:"uppercase"}}>Reschedule Requests</p>
                </div>
              </div>
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                ...
                <button onClick={loadReschedules}
                  style={{padding:"8px 14px",...sf,fontSize:6,letterSpacing:"0.2em",textTransform:"uppercase",background:"transparent",border:`1px solid ${T.border}`,color:T.muted,cursor:"pointer",transition:"all 0.2s"}}
                  onMouseEnter={e=>{e.currentTarget.style.borderColor=T.amber;e.currentTarget.style.color=T.amber;}}
                  onMouseLeave={e=>{e.currentTarget.style.borderColor=T.border;e.currentTarget.style.color=T.muted;}}>
                  Refresh
                </button>
            ...
            {reschedLoading?(
              <div style={{padding:"48px",textAlign:"center"}}>
                <div style={{width:18,height:18,border:`2px solid rgba(245,158,11,0.2)`,borderTopColor:T.amber,borderRadius:"50%",animation:"spin 0.8s linear infinite",margin:"0 auto"}}/>
              </div>
            ):reschedules.length===0?(
              <div style={{padding:"64px 20px",textAlign:"center",border:`1px solid ${T.border}`,background:T.surface,
                clipPath:"polygon(0 0,calc(100% - 16px) 0,100% 16px,100% 100%,16px 100%,0 calc(100% - 16px))"
              ...
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                ...
                {[...reschedules.filter(r=>r.status==="pending"), ...reschedules.filter(r=>r.status!=="pending")].map(rr=>{
                  ...
                  const isRejected = rr.status==="rejected";
                  const statusColor = isPending?"#f59e0b":isAccepted?"#22c55e":"#f87171";
                  const statusBg    = isPending?"rgba(245,158,11,0.08)":isAccepted?"rgba(34,197,94,0.08)":"rgba(248,113,113,0.06)";
                  const statusBdr   = isPending?"rgba(245,158,11,0.25)":isAccepted?"rgba(34,197,94,0.2)":"rgba(248,113,113,0.15)";

                  return(
                    <div key={rr.id} style={{background:T.surface,border:`1px solid ${isPending?T.amberBorder:T.border}`,overflow:"hidden",
                      clipPath:"polygon(0 0,calc(100% - 12px) 0,100% 12px,100% 100%,12px 100%,0 calc(100% - 12px))",
                      transition:"border-color 0.2s"
                    }}>
                      {/* Color bar */}
                      <div style={{height:2,background:`linear-gradient(to right,${statusColor},transparent)`,opacity:0.8}}/>

                      <div style={{padding:"18px 20px"}}>
                        {/* Top row — client + status */}
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16,flexWrap:"wrap",gap:10}}>
                          <div style={{display:"flex",alignItems:"center",gap:12}}>
                            <div style={{width:38,height:38,background:T.amberDim,border:`1px solid ${T.amberBorder}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                        ...
                        <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr auto 1fr",gap:12,alignItems:"center",marginBottom:isPending?16:0}}>
                          {/* Original */}
                          ...
                          <div style={{textAlign:"center",padding:isMobile?"8px 0":"0"}}>
                            <span style={{...sf,fontSize:18,color:T.amber,display:"block"}}>→</span>
                            {isMobile&&<p style={{...mono,fontSize:8,color:"#52525b",marginTop:4}}>Requested change</p>}
                          </div>

                          {/* Requested */}
                          <div style={{padding:"14px 16px",background:isPending?"rgba(245,158,11,0.05)":isAccepted?"rgba(34,197,94,0.06)":"rgba(248,113,113,0.04)",
                            border:`1px solid ${isPending?T.amberBorder:isAccepted?"rgba(34,197,94,0.2)":"rgba(248,113,113,0.12)"}`
                          }}>
                            <p style={{...mono,fontSize:8,color:isPending?"rgba(245,158,11,0.5)":isAccepted?"rgba(34,197,94,0.5)":"rgba(248,113,113,0.4)",letterSpacing:"0.35em",textTransform:"uppercase",marginBottom:8}}>
                        ...
                        {isPending&&(
                          <div style={{display:"flex",gap:10,flexWrap:"wrap",paddingTop:16,borderTop:`1px solid rgba(255,255,255,0.06)`}}>
                            <button onClick={async()=>{
                              ...
                              style={{padding:"12px 24px",background:"rgba(34,197,94,0.1)",border:"1px solid rgba(34,197,94,0.35)",color:"#4ade80",...sf,fontSize:7,fontWeight:700,letterSpacing:"0.2em",textTransform:"uppercase",cursor:"pointer",transition:"all 0.2s",flex:1,
                                clipPath:"polygon(0 0,calc(100% - 7px) 0,100% 7px,100% 100%,7px 100%,0 calc(100% - 7px))"
                              }}
                              onMouseEnter={e=>e.currentTarget.style.background="rgba(34,197,94,0.2)"}
                              onMouseLeave={e=>e.currentTarget.style.background="rgba(34,197,94,0.1)"}>
                            ...
                            <button onClick={async()=>{
                              ...
                              style={{padding:"12px 24px",background:"rgba(248,113,113,0.06)",border:"1px solid rgba(248,113,113,0.25)",color:"#f87171",...sf,fontSize:7,fontWeight:700,letterSpacing:"0.2em",textTransform:"uppercase",cursor:"pointer",transition:"all 0.2s",flex:1,
                                clipPath:"polygon(0 0,calc(100% - 7px) 0,100% 7px,100% 100%,7px 100%,0 calc(100% - 7px))"
                              }}
        ...
        {activeTab==="walkin"&&(
          <div className="bd-enter" style={{maxWidth:600}}>
            ...
            <div style={{marginBottom:28}}>
              <p style={{...mono,fontSize:8,color:"rgba(245,158,11,0.5)",letterSpacing:"0.5em",textTransform:"uppercase",marginBottom:6}}>HEADZ UP · FRONT DESK</p>
              <h2 style={{...sf,fontSize:22,fontWeight:900,textTransform:"uppercase",letterSpacing:"-0.03em",margin:0}}>
                Walk-In<br/><span style={{color:T.amber,fontStyle:"italic"}}>Check-In_</span>
              </h2>
              <p style={{...mono,fontSize:12,color:T.muted,marginTop:10,lineHeight:1.7}}>
                Someone just walked in? Fill this out and they're in the system instantly. If they give a phone or email, they'll get a welcome text or email automatically.
              </p>
            </div>

            {/* Success card */}
            {wiSuccess&&(
              <div style={{marginBottom:24,padding:"20px",background:"rgba(34,197,94,0.06)",border:"1px solid rgba(34,197,94,0.2)",
                clipPath:"polygon(0 0,calc(100% - 10px) 0,100% 10px,100% 100%,10px 100%,0 calc(100% - 10px))"
              }}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:16}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                ...
                <div>
                  <label style={{...sf,fontSize:6,letterSpacing:"0.4em",color:T.muted,textTransform:"uppercase",display:"block",marginBottom:8}}>Service *</label>
                  <select value={wiSvc} onChange={e=>setWiSvc(e.target.value)}
                    style={{width:"100%",background:T.bg,border:`1px solid ${T.border}`,padding:"13px 14px",color:wiSvc?"white":T.muted,...mono,fontSize:13,outline:"none",cursor:"pointer",colorScheme:"dark"}}
                    onFocus={e=>e.target.style.borderColor=T.amber}
                    onBlur={e=>e.target.style.borderColor=T.border}>
              ...
              {/* Date */}
              <div>
                <label style={{...sf,fontSize:6,letterSpacing:"0.4em",color:T.muted,textTransform:"uppercase",display:"block",marginBottom:8}}>Date *</label>
                <input type="date" value={wiDate} min={today} onChange={e=>{setWiDate(e.target.value);setWiTime("");}}
                  style={{width:"100%",background:T.bg,border:`1px solid ${T.border}`,padding:"13px 14px",color:"white",...mono,fontSize:13,outline:"none",colorScheme:"dark"}}
                  onFocus={e=>e.target.style.borderColor=T.amber}
                  onBlur={e=>e.target.style.borderColor=T.border}/>
              </div>
                ...
                <div>
                  ...
                  {wiSlotsLoad ? (
                    <div style={{display:"flex",alignItems:"center",gap:8,padding:"14px",background:T.surface,border:`1px solid ${T.border}`}}>
                      <div style={{width:14,height:14,border:"2px solid rgba(245,158,11,0.2)",borderTopColor:T.amber,borderRadius:"50%",animation:"spin 0.8s linear infinite"}}/>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:5}}>
                      {wiSlots.map(s=>{
                        ...
                        return(
                          <button key={s} onClick={()=>!isBooked&&setWiTime(display)} disabled={isBooked}
                            style={{padding:"9px 4px",position:"relative",overflow:"hidden",
                              ...
                              background:isSel?"rgba(245,158,11,0.15)":isBooked?"rgba(239,68,68,0.03)":"rgba(245,158,11,0.04)",
                              color:isSel?T.amber:isBooked?"#2a2a2a":"#a1a1aa",
                            ...
                            {isBooked&&(
                              <svg style={{position:"absolute",inset:0,width:"100%",height:"100%",pointerEvents:"none"}} viewBox="0 0 80 32" preserveAspectRatio="none">
                                <line x1="4" y1="28" x2="76" y2="4" stroke="rgba(239,68,68,0.6)" strokeWidth="1.5"
              ...
              <div style={{borderTop:`1px solid ${T.border}`,paddingTop:16}}>
                <p style={{...sf,fontSize:6,letterSpacing:"0.4em",color:T.muted,textTransform:"uppercase",marginBottom:14}}>Client Info</p>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
                  <div>
                    <label style={{...sf,fontSize:6,letterSpacing:"0.4em",color:T.muted,textTransform:"uppercase",display:"block",marginBottom:8}}>Name *</label>
                    <input value={wiName} onChange={e=>setWiName(e.target.value)} placeholder="John Smith"
                      style={{width:"100%",background:T.bg,border:`1px solid ${T.border}`,padding:"13px 14px",color:"white",...mono,fontSize:13,outline:"none"}}
                      onFocus={e=>e.target.style.borderColor=T.amber}
                      onBlur={e=>e.target.style.borderColor=T.border}/>
                  </div>
                  <div>
                    <label style={{...sf,fontSize:6,letterSpacing:"0.4em",color:T.muted,textTransform:"uppercase",display:"block",marginBottom:8}}>Phone <span style={{color:T.dim}}>(for welcome text)</span></label>
                    <input type="tel" value={wiPhone} onChange={e=>setWiPhone(e.target.value)} placeholder="601-555-0100"
                      style={{width:"100%",background:T.bg,border:`1px solid ${T.border}`,padding:"13px 14px",color:"white",...mono,fontSize:13,outline:"none"}}
                      onFocus={e=>e.target.style.borderColor=T.amber}
                      onBlur={e=>e.target.style.borderColor=T.border}/>
                  </div>
                ...
                <div style={{marginBottom:12}}>
                  <label style={{...sf,fontSize:6,letterSpacing:"0.4em",color:T.muted,textTransform:"uppercase",display:"block",marginBottom:8}}>Email <span style={{color:T.dim}}>(for welcome email)</span></label>
                  <input type="email" value={wiEmail} onChange={e=>setWiEmail(e.target.value)} placeholder="client@email.com"
                    style={{width:"100%",background:T.bg,border:`1px solid ${T.border}`,padding:"13px 14px",color:"white",...mono,fontSize:13,outline:"none"}}
                    onFocus={e=>e.target.style.borderColor=T.amber}
                    onBlur={e=>e.target.style.borderColor=T.border}/>
                </div>
                <div>
                  <label style={{...sf,fontSize:6,letterSpacing:"0.4em",color:T.muted,textTransform:"uppercase",display:"block",marginBottom:8}}>Notes</label>
                  <input value={wiNotes} onChange={e=>setWiNotes(e.target.value)} placeholder="Style request, preferences..."
                    style={{width:"100%",background:T.bg,border:`1px solid ${T.border}`,padding:"13px 14px",color:"white",...mono,fontSize:13,outline:"none"}}
                    onFocus={e=>e.target.style.borderColor=T.amber}
                    onBlur={e=>e.target.style.borderColor=T.border}/>
                </div>
              ...
              {(wiPhone||wiEmail)&&wiName&&(
                <div style={{padding:"12px 14px",background:"rgba(245,158,11,0.04)",border:"1px solid rgba(245,158,11,0.15)"}}>
                  <p style={{...mono,fontSize:8,color:"rgba(245,158,11,0.5)",letterSpacing:"0.4em",textTransform:"uppercase",marginBottom:6}}>Preview — Welcome Message</p>
                  <p style={{...mono,fontSize:11,color:"#a1a1aa",lineHeight:1.7,fontStyle:"italic"}}>
                    "Hey {wiName}! Welcome to HEADZ UP Barbershop 🔥 You're officially part of the family. We'll take care of you today — see you in the chair!"
                  </p>
                </div>
              )}
              ...
              <button onClick={handleWalkIn} disabled={wiLoading||!wiName.trim()||!wiSvc||!wiTime||!wiBarber}
                ...
                {wiLoading
                  ? <span style={{display:"flex",alignItems:"center",justifyContent:"center",gap:10}}>
                      <span style={{width:14,height:14,border:"2px solid #3f3f46",borderTopColor:"#a1a1aa",borderRadius:"50%",display:"inline-block",animation:"spin 0.7s linear infinite"}}/>
                      Adding Walk-In...
                    </span>
        ...
        {activeTab==="waitlist"&&(
          <div className="bd-enter">
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24,flexWrap:"wrap",gap:12}}>
              <div style={{display:"flex",alignItems:"center",gap:14}}>
                <div style={{width:4,height:28,background:"linear-gradient(to bottom,#ef4444,#f59e0b)"}}/>
                <div>
                  <p style={{...mono,fontSize:7,color:"rgba(245,158,11,0.5)",letterSpacing:"0.5em",textTransform:"uppercase",marginBottom:2}}>HEADZ UP · QUEUE</p>
                  <p style={{...sf,fontSize:13,fontWeight:900,textTransform:"uppercase"}}>Waitlist</p>
                </div>
              </div>
              <div style={{padding:"8px 14px",background:"rgba(245,158,11,0.08)",border:"1px solid rgba(245,158,11,0.2)"}}>
            ...
            {waitlist.length===0?(
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {waitlist.map((w,i)=>(
                  <div key={w.id} style={{background:T.surface,border:`1px solid ${w.notified?T.border:T.amberBorder}`,overflow:"hidden",
                    clipPath:"polygon(0 0,calc(100% - 10px) 0,100% 10px,100% 100%,10px 100%,0 calc(100% - 10px))"
                  }}>
                    <div style={{height:2,background:w.notified?"rgba(255,255,255,0.05)":`linear-gradient(to right,#ef4444,#f59e0b)`}}/>
                    <div style={{padding:"16px 18px",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12}}>
                      <div style={{display:"flex",alignItems:"center",gap:14}}>
                        {/* Queue number */}
                        <div style={{width:36,height:36,background:w.notified?T.surface2:T.amberDim,border:`1px solid ${w.notified?T.border:T.amberBorder}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                      ...
                      <div style={{display:"flex",gap:8}}>
                        <button onClick={()=>!w.notified&&notifyWaitlist(w.id)} disabled={w.notified}
                          style={{padding:"9px 18px",...sf,fontSize:7,letterSpacing:"0.15em",textTransform:"uppercase",
                            background:w.notified?T.surface2:T.amber,
                            border:`1px solid ${w.notified?T.border:T.amber}`,
                            color:w.notified?T.muted:"black",
                            cursor:w.notified?"default":"pointer",transition:"all 0.2s",
                            clipPath:"polygon(0 0,calc(100% - 5px) 0,100% 5px,100% 100%,5px 100%,0 calc(100% - 5px))"
                          }}>
          ...
          <div className="bd-enter">
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24,flexWrap:"wrap",gap:12}}>
              <div style={{display:"flex",alignItems:"center",gap:14}}>
                <div style={{width:4,height:28,background:"linear-gradient(to bottom,#ef4444,#f59e0b)"}}/>
                <div>
                  <p style={{...mono,fontSize:7,color:"rgba(245,158,11,0.5)",letterSpacing:"0.5em",textTransform:"uppercase",marginBottom:2}}>HEADZ UP · ROSTER</p>
                  <p style={{...sf,fontSize:13,fontWeight:900,textTransform:"uppercase"}}>Client Book</p>
                </div>
              </div>
              {/* Stats pills */}
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                {[
                  ...
                  {label:"VIP",    val:clients.filter(c=>c.is_vip).length,                            color:"#a78bfa"},
                  {label:"Strikes",val:clients.filter(c=>(c.strike_count||0)>0).length,               color:"#f87171"},
            ...
            <input value={clientSearch} onChange={e=>setClientSearch(e.target.value)} placeholder="Search by name or email..."
              style={{width:"100%",background:T.surface,border:`1px solid ${T.border}`,padding:"13px 16px",color:"white",fontSize:15,...mono,outline:"none",marginBottom:16,transition:"border-color 0.2s"}}
              onFocus={e=>e.target.style.borderColor=T.amber}
              onBlur={e=>e.target.style.borderColor=T.border}/>
            ...
            {clientDetail?(
              ...
              <div>
                ...
                <div style={{background:T.surface,border:`1px solid ${T.amberBorder}`,clipPath:"polygon(0 0,calc(100% - 14px) 0,100% 14px,100% 100%,14px 100%,0 calc(100% - 14px))"}}>
                  {/* Client header */}
                  <div style={{padding:"24px 22px",borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:16}}>
                    <div style={{display:"flex",alignItems:"center",gap:16}}>
                      <div style={{width:56,height:56,background:clientDetail.is_vip?T.amber:T.amberDim,border:`2px solid ${clientDetail.is_vip?T.amber:T.amberBorder}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,clipPath:"polygon(0 0,calc(100% - 8px) 0,100% 8px,100% 100%,8px 100%,0 calc(100% - 8px))"}}>
                        <span style={{...sf,fontSize:22,fontWeight:900,color:clientDetail.is_vip?"black":T.amber}}>{clientDetail.name?.charAt(0)?.toUpperCase()||"?"}</span>
                      </div>
                      <div>
                        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:4}}>
                          <p style={{...sf,fontSize:14,fontWeight:900,textTransform:"uppercase"}}>{clientDetail.name}</p>
                    ...
                    <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                      <button onClick={async()=>{
                        try{
                          await API.patch(`barber/clients/${clientDetail.id}/`,{is_vip:!clientDetail.is_vip});
                          setClientDetail(p=>({...p,is_vip:!p.is_vip}));
                          setClients(p=>p.map(c=>c.id===clientDetail.id?{...c,is_vip:!c.is_vip}:c));
                          showToast(clientDetail.is_vip?"Removed VIP status":"⭐ Marked as VIP!");
                        }catch{showToast("Error.","error");}
                      ...
                      }} style={{padding:"9px 16px",...sf,fontSize:7,letterSpacing:"0.15em",textTransform:"uppercase",
                        background:blockList.includes(clientDetail.username||clientDetail.name)?T.redDim:"transparent",
                        border:`1px solid ${blockList.includes(clientDetail.username||clientDetail.name)?T.redBorder:T.border}`,
                        color:blockList.includes(clientDetail.username||clientDetail.name)?T.red:T.muted,
                        cursor:"pointer",transition:"all 0.2s"}}>
                        🚫 {blockList.includes(clientDetail.username||clientDetail.name)?"Unblock":"Block"}
                      </button>
                    </div>
                  </div>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",borderBottom:`1px solid ${T.border}`}}>
                    {[
                      {label:"Visits",   val:clientDetail.total_visits||0,   color:T.amber},
                      {label:"Completed",val:clientDetail.completed||0,       color:T.green},
                      {label:"No Shows", val:clientDetail.no_shows||0,        color:T.red},
                      {label:"Deposit",  val:`$${clientDetail.deposit_fee||"10.00"}`, color:"#a78bfa"},
                    ].map(({label,val,color})=>(
                      <div key={label} style={{padding:"16px 12px",borderRight:`1px solid ${T.border}`,textAlign:"center"}}>
                        <p style={{...sf,fontSize:18,fontWeight:900,color,lineHeight:1,marginBottom:4}}>{val}</p>
                        <p style={{...mono,fontSize:8,color:"#71717a",textTransform:"uppercase",letterSpacing:"0.2em"}}>{label}</p>
                      </div>
                    ))}
                  </div>
                  ...
                  <div style={{padding:"20px 22px"}}>
                    <p style={{...sf,fontSize:6,letterSpacing:"0.4em",color:T.muted,textTransform:"uppercase",marginBottom:10}}>Barber Notes</p>
                    <textarea value={clientNotes} onChange={e=>setClientNotes(e.target.value)} rows={3}
                      placeholder="Preferred style, allergies, notes..."
                      style={{width:"100%",background:T.bg,border:`1px solid ${T.border}`,padding:"12px 14px",color:"white",fontSize:14,...mono,outline:"none",resize:"vertical",marginBottom:12,borderRadius:0,lineHeight:1.6}}
                      onFocus={e=>e.target.style.borderColor=T.amber}
                      onBlur={e=>e.target.style.borderColor=T.border}/>
                    <button onClick={async()=>{
                      setSavingCN(true);
                      try{await API.patch(`barber/clients/${clientDetail.id}/`,{notes:clientNotes});showToast("Notes saved.");}
                      catch{showToast("Error.","error");}
                      finally{setSavingCN(false);}
                    }} disabled={savingCN}
                      style={{padding:"12px 24px",background:savingCN?T.deep:T.amber,color:savingCN?T.muted:"black",...sf,fontSize:8,fontWeight:700,letterSpacing:"0.2em",textTransform:"uppercase",border:"none",cursor:savingCN?"not-allowed":"pointer",transition:"all 0.2s",clipPath:"polygon(0 0,calc(100% - 7px) 0,100% 7px,100% 100%,7px 100%,0 calc(100% - 7px))"}}>
              ...
              /* ── CLIENT LIST ── */
              <div style={{display:"flex",flexDirection:"column",gap:6}}>
                {clients.filter(c=>!clientSearch||c.name?.toLowerCase().includes(clientSearch.toLowerCase())||c.email?.toLowerCase().includes(clientSearch.toLowerCase())).map(c=>{
                  const isBlocked = blockList.includes(c.username||c.name);
                  const strikes = c.strike_count||0;
                  return(
                    <div key={c.id} onClick={()=>{setClientDetail(c);setClientNotes(c.notes||"");}}
                      style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 18px",background:isBlocked?"rgba(248,113,113,0.03)":T.surface,border:`1px solid ${c.is_vip?T.amberBorder:isBlocked?T.redBorder:T.border}`,cursor:"pointer",transition:"all 0.2s",clipPath:"polygon(0 0,calc(100% - 8px) 0,100% 8px,100% 100%,8px 100%,0 calc(100% - 8px))"}}
                      onMouseEnter={e=>{e.currentTarget.style.borderColor=T.amber;e.currentTarget.style.background=T.amberDim;}}
                      onMouseLeave={e=>{e.currentTarget.style.borderColor=c.is_vip?T.amberBorder:isBlocked?T.redBorder:T.border;e.currentTarget.style.background=isBlocked?"rgba(248,113,113,0.03)":T.surface;}}>
                      <div style={{display:"flex",alignItems:"center",gap:14}}>
                        <div style={{width:40,height:40,background:c.is_vip?T.amber:isBlocked?T.redDim:T.surface2,border:`1px solid ${c.is_vip?T.amber:isBlocked?T.redBorder:T.border}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,clipPath:"polygon(0 0,calc(100% - 5px) 0,100% 5px,100% 100%,5px 100%,0 calc(100% - 5px))"}}>
                          <span style={{...sf,fontSize:15,fontWeight:900,color:c.is_vip?"black":isBlocked?T.red:T.muted}}>{c.name?.charAt(0)?.toUpperCase()||"?"}</span>
                        </div>
                        <div>
          ...
          <div className="bd-enter">
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24,flexWrap:"wrap",gap:12}}>
              <div style={{display:"flex",alignItems:"center",gap:14}}>
                <div style={{width:4,height:28,background:"linear-gradient(to bottom,#ef4444,#f59e0b)"}}/>
                <div>
                  <p style={{...mono,fontSize:7,color:"rgba(245,158,11,0.5)",letterSpacing:"0.5em",textTransform:"uppercase",marginBottom:2}}>HEADZ UP · ANALYTICS</p>
                  <p style={{...sf,fontSize:13,fontWeight:900,textTransform:"uppercase"}}>Reports</p>
                </div>
              </div>
              <div style={{display:"flex",gap:6}}>
                {["week","month","year","all"].map(k=>(
                  <button key={k} onClick={()=>setReportPeriod(k)}
                    style={{padding:"8px 14px",...sf,fontSize:6.5,letterSpacing:"0.15em",textTransform:"uppercase",
            ...
            {(!reports||reportsLoad)?(
              ...
              <>
                ...
                <div style={{display:"grid",gridTemplateColumns:isMobile?"repeat(2,1fr)":"repeat(4,1fr)",gap:8,marginBottom:12}}>
                  {[
                    {label:"Total Bookings", value:reports.summary?.total||0,                                    icon:"📅", color:T.amber,   bg:T.amberDim,   border:T.amberBorder},
                    {label:"Completed",      value:reports.summary?.completed||0,                               icon:"✓",  color:T.green,   bg:T.greenDim,   border:T.greenBorder},
                    {label:"Cancelled",      value:reports.summary?.cancelled||0,                               icon:"✕",  color:"#f87171", bg:"rgba(248,113,113,0.06)", border:"rgba(248,113,113,0.2)"},
                    {label:"No Shows",       value:reports.summary?.no_shows||0,                                icon:"⚡", color:"#f59e0b", bg:"rgba(245,158,11,0.06)",  border:"rgba(245,158,11,0.2)"},
                  ].map(({label,value,icon,color,bg,border})=>(
                    <div key={label} style={{padding:isMobile?"16px 12px":"20px 16px",background:bg,border:`1px solid ${border}`,position:"relative",overflow:"hidden",clipPath:"polygon(0 0,calc(100% - 8px) 0,100% 8px,100% 100%,8px 100%,0 calc(100% - 8px))"}}>
                      <div style={{position:"absolute",top:8,right:12,fontSize:20,opacity:0.3}}>{icon}</div>
                      <p style={{...mono,fontSize:8,color:color,textTransform:"uppercase",letterSpacing:"0.3em",marginBottom:8,opacity:0.8}}>{label}</p>
                      <p style={{...sf,fontSize:isMobile?28:34,fontWeight:900,color,lineHeight:1,margin:0}}>{value}</p>
                    </div>
                  ))}
                </div>
                ...
                <div style={{display:"grid",gridTemplateColumns:isMobile?"repeat(2,1fr)":"repeat(4,1fr)",gap:8,marginBottom:20}}>
                  {[
                    {label:"Rescheduled",   value:reports.summary?.reschedules||0,          icon:"↻",  color:"#a78bfa"},
                    {label:"On Strike",     value:reports.summary?.clients_on_strike||0,    icon:"⚡", color:"#f87171"},
                    {label:"Walk-ins",      value:reports.summary?.walk_ins||0,              icon:"🚶", color:T.amber},
                    {label:"No-show Rate",  value:`${reports.summary?.no_show_rate||0}%`,   icon:"📊", color:"#f59e0b"},
                  ].map(({label,value,icon,color})=>(
                    <div key={label} style={{padding:isMobile?"14px 12px":"16px 16px",background:T.surface,border:`1px solid ${T.border}`,position:"relative",overflow:"hidden",clipPath:"polygon(0 0,calc(100% - 6px) 0,100% 6px,100% 100%,6px 100%,0 calc(100% - 6px))"}}>
                      <div style={{position:"absolute",top:6,right:10,fontSize:16,opacity:0.2}}>{icon}</div>
                      <p style={{...mono,fontSize:7,color:"#71717a",textTransform:"uppercase",letterSpacing:"0.25em",marginBottom:6}}>{label}</p>
                      <p style={{...sf,fontSize:isMobile?22:26,fontWeight:900,color,lineHeight:1,margin:0}}>{value}</p>
                    </div>
                  ))}
                </div>
                ...
                <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"repeat(3,1fr)",gap:8,marginBottom:24}}>
                  {[
                    {label:"Total Revenue",  value:`$${reports.summary?.total_revenue||"0.00"}`,  color:T.amber},
                    {label:"Online Revenue", value:`$${reports.summary?.online_revenue||"0.00"}`,  color:"#a78bfa"},
                    {label:"Shop Revenue",   value:`$${reports.summary?.shop_revenue||"0.00"}`,    color:T.green},
                  ].map(({label,value,color})=>(
                    <div key={label} style={{padding:"18px 20px",background:T.surface,border:`1px solid ${T.border}`,clipPath:"polygon(0 0,calc(100% - 8px) 0,100% 8px,100% 100%,8px 100%,0 calc(100% - 8px))"}}>
                      <p style={{...mono,fontSize:8,color:"#71717a",textTransform:"uppercase",letterSpacing:"0.3em",marginBottom:6}}>{label}</p>
                      <p style={{...sf,fontSize:26,fontWeight:900,color,lineHeight:1}}>{value}</p>
                    </div>
                  ))}
                </div>

                {/* ── COMPLETION RATE VISUAL ── */}
                <div style={{marginBottom:20,background:T.surface,border:`1px solid ${T.border}`,padding:"20px",clipPath:"polygon(0 0,calc(100% - 10px) 0,100% 10px,100% 100%,10px 100%,0 calc(100% - 10px))"}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}>
                    <p style={{...sf,fontSize:7,letterSpacing:"0.3em",color:T.muted,textTransform:"uppercase"}}>Completion Rate</p>
                    <p style={{...sf,fontSize:16,fontWeight:900,color:T.green}}>{reports.summary?.completion_rate||0}%</p>
                  </div>
                ...
                {reports.daily?.length>0&&(
                  <div style={{marginBottom:20,background:T.surface,border:`1px solid ${T.border}`,padding:"20px",clipPath:"polygon(0 0,calc(100% - 10px) 0,100% 10px,100% 100%,10px 100%,0 calc(100% - 10px))"}}>
                    <p style={{...sf,fontSize:7,letterSpacing:"0.3em",color:T.muted,textTransform:"uppercase",marginBottom:16}}>Revenue — Last 14 Days</p>
                    {(()=>{
                      const last14 = reports.daily.slice(-14);
                      const maxRev = Math.max(...last14.map(d=>d.revenue||0), 1);
                      return(
                          ...
                          {last14.map((d,i)=>{
                            ...
                            return(
                              <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
                                <span style={{...mono,fontSize:7,color:T.amber,opacity:d.revenue>0?1:0}}>${d.revenue||0}</span>
                                <div style={{width:"100%",background:isToday?T.amber:`linear-gradient(to top,rgba(245,158,11,0.6),rgba(245,158,11,0.3))`,height:`${Math.max(pct,3)}%`,transition:"height 0.5s ease",border:isToday?`1px solid ${T.amber}`:"none"}}/>
                                <span style={{...mono,fontSize:isMobile?6:7,color:isToday?T.amber:T.muted,whiteSpace:"nowrap"}}>{d.label?.split(" ")[1]||d.label}</span>
                              </div>
                            );
                          })}
                        </div>
                ...
                {(()=>{
                  ...
                  const completed  = Math.round(((s.completed||0)/total)*100);
                  const cancelled  = Math.round(((s.cancelled||0)/total)*100);
                  const no_shows   = Math.round(((s.no_shows||0)/total)*100);
                  const confirmed  = 100 - completed - cancelled - no_shows;
                  return(
                    <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:12,marginBottom:20}}>
                      {/* Donut chart */}
                      <div style={{background:T.surface,border:`1px solid ${T.border}`,padding:"20px",clipPath:"polygon(0 0,calc(100% - 10px) 0,100% 10px,100% 100%,10px 100%,0 calc(100% - 10px))"}}>
                        <p style={{...sf,fontSize:7,letterSpacing:"0.3em",color:T.muted,textTransform:"uppercase",marginBottom:16}}>Booking Breakdown</p>
                          ...
                          <svg width="100" height="100" viewBox="0 0 100 100" style={{flexShrink:0}}>
                            {(()=>{
                              ...
                              return data.map((d,i)=>{
                                ...
                                const el = <circle key={i} cx={cx} cy={cy} r={r} fill="none" stroke={d.color} strokeWidth="14" strokeDasharray={`${dash} ${circ-dash}`} strokeDashoffset={-offset*circ/100} transform="rotate(-90 50 50)" opacity="0.9"/>;
                                offset += d.pct;
                                return el;
                              });
                            })()}
                            <circle cx="50" cy="50" r="21" fill={T.bg}/>
                            <text x="50" y="47" textAnchor="middle" style={{fill:"white",fontSize:"11px",fontWeight:900}}>{s.total||0}</text>
                            <text x="50" y="57" textAnchor="middle" style={{fill:"#71717a",fontSize:"7px"}}>total</text>
                          </svg>
                          {/* Legend */}
                          <div style={{display:"flex",flexDirection:"column",gap:8}}>
                            {[
                              {label:"Completed", pct:completed, color:"#22c55e"},
                              {label:"Confirmed", pct:confirmed, color:T.amber},
                              {label:"Cancelled", pct:cancelled, color:"#f87171"},
                              {label:"No Show",   pct:no_shows,  color:"#ef4444"},
                            ].map(({label,pct,color})=>(
                      ...
                      {/* Busiest hours bar */}
                      <div style={{background:T.surface,border:`1px solid ${T.border}`,padding:"20px",clipPath:"polygon(0 0,calc(100% - 10px) 0,100% 10px,100% 100%,10px 100%,0 calc(100% - 10px))"}}>
                        <p style={{...sf,fontSize:7,letterSpacing:"0.3em",color:T.muted,textTransform:"uppercase",marginBottom:16}}>Busiest Hours</p>
                        {reports.busiest_hours?.length>0?(
                          <div style={{display:"flex",alignItems:"flex-end",gap:3,height:80}}>
                            {reports.busiest_hours.map((h,i)=>{
                              const maxC=Math.max(...reports.busiest_hours.map(x=>x.bookings||0),1);
                              const cnt=h.bookings||0;
                              const pct=cnt/maxC*100;
                              return(
                                <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
                                  <div style={{width:"100%",background:pct>60?T.amber:"rgba(245,158,11,0.35)",height:`${Math.max(pct,4)}%`,transition:"height 0.5s ease"}}/>
                                  <span style={{...mono,fontSize:6,color:T.muted,whiteSpace:"nowrap"}}>{h.label||h.hour}</span>
                                </div>
                              );
                            })}
                          </div>
                ...
                {reports.services?.length>0&&(
                  <div style={{marginBottom:20,background:T.surface,border:`1px solid ${T.border}`,padding:"20px",clipPath:"polygon(0 0,calc(100% - 10px) 0,100% 10px,100% 100%,10px 100%,0 calc(100% - 10px))"}}>
                    <p style={{...sf,fontSize:7,letterSpacing:"0.3em",color:T.muted,textTransform:"uppercase",marginBottom:16}}>Top Services</p>
                    <div style={{display:"flex",flexDirection:"column",gap:10}}>
                      {reports.services.map((s,i)=>{
                        const maxB = reports.services[0]?.bookings||1;
                        const pct  = Math.round(((s.bookings||0)/maxB)*100);
                        return(
                          <div key={i}>
                            ...
                            <div style={{height:3,background:T.border,borderRadius:2,overflow:"hidden"}}>
                              <div style={{height:"100%",width:`${pct}%`,background:`linear-gradient(to right,#f59e0b,#fbbf24)`,transition:"width 1s ease"}}/>
                            </div>
                          </div>
                        );
                      })}
                ...
                {reports.top_clients?.length>0&&(
                  <div style={{marginBottom:20,background:T.surface,border:`1px solid ${T.border}`,padding:"20px",clipPath:"polygon(0 0,calc(100% - 10px) 0,100% 10px,100% 100%,10px 100%,0 calc(100% - 10px))"}}>
                    <p style={{...sf,fontSize:7,letterSpacing:"0.3em",color:T.muted,textTransform:"uppercase",marginBottom:16}}>Top Clients</p>
                    <div style={{display:"flex",flexDirection:"column",gap:8}}>
        ...
        {activeTab==="newsletter"&&(
          <div className="bd-enter">
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24,flexWrap:"wrap",gap:12}}>
              <div style={{display:"flex",alignItems:"center",gap:14}}>
                <div style={{width:4,height:28,background:"linear-gradient(to bottom,#ef4444,#f59e0b)",flexShrink:0}}/>
                <div>
                  <p style={{...mono,fontSize:7,color:"rgba(245,158,11,0.5)",letterSpacing:"0.5em",textTransform:"uppercase",marginBottom:2}}>HEADZ UP · UPDATES</p>
                  <p style={{...sf,fontSize:13,fontWeight:900,textTransform:"uppercase"}}>Newsletter Posts</p>
                </div>
              </div>
            ...
            {/* Create / Edit form */}
            <div style={{background:T.surface,border:`1px solid ${nlEditing?"#635bff":"rgba(245,158,11,0.2)"}`,padding:"22px",marginBottom:24,
              clipPath:"polygon(0 0,calc(100% - 12px) 0,100% 12px,100% 100%,12px 100%,0 calc(100% - 12px))"
            }}>
              <p style={{...sf,fontSize:7,letterSpacing:"0.4em",color:nlEditing?"#a78bfa":T.amber,textTransform:"uppercase",marginBottom:16}}>
              ...
              <input value={nlForm.title} onChange={e=>setNlForm(p=>({...p,title:e.target.value}))}
                ...
                style={{width:"100%",background:T.bg,border:`1px solid ${T.border}`,padding:"13px 16px",color:"white",fontSize:15,...mono,outline:"none",marginBottom:10,transition:"border-color 0.2s"}}
                onFocus={e=>e.target.style.borderColor=T.amber}
                onBlur={e=>e.target.style.borderColor=T.border}/>

              {/* Category + emoji row */}
              <div style={{display:"flex",gap:8,marginBottom:10}}>
                <select value={nlForm.category} onChange={e=>setNlForm(p=>({...p,category:e.target.value}))}
                  style={{flex:2,background:T.bg,border:`1px solid ${T.border}`,padding:"12px 14px",color:"white",fontSize:14,...mono,outline:"none",cursor:"pointer"}}>
                  {[["deal","💸 Deal / Discount"],["promo","🎯 Promotion"],["update","📢 Shop Update"],["event","🎉 Event"],["general","✂️ General"]].map(([v,l])=>(
                    <option key={v} value={v}>{l}</option>
                  ))}
                </select>
                <input value={nlForm.emoji} onChange={e=>setNlForm(p=>({...p,emoji:e.target.value}))}
                  placeholder="Emoji"
                  style={{width:60,background:T.bg,border:`1px solid ${T.border}`,padding:"12px",color:"white",fontSize:20,outline:"none",textAlign:"center"}}/>
                <div style={{display:"flex",alignItems:"center",gap:8,padding:"12px 14px",background:T.bg,border:`1px solid ${T.border}`,cursor:"pointer"}}
                  onClick={()=>setNlForm(p=>({...p,pinned:!p.pinned}))}>
                  <div style={{width:16,height:16,border:`2px solid ${nlForm.pinned?T.amber:"rgba(255,255,255,0.2)"}`,background:nlForm.pinned?"rgba(245,158,11,0.15)":"transparent",display:"flex",alignItems:"center",justifyContent:"center"}}>
              ...
              {/* Body */}
              <textarea value={nlForm.body} onChange={e=>setNlForm(p=>({...p,body:e.target.value}))}
                placeholder="Write your post — deals, promos, shop news, hours changes..."
                rows={5}
                style={{width:"100%",background:T.bg,border:`1px solid ${T.border}`,padding:"13px 16px",color:"white",fontSize:14,...mono,outline:"none",resize:"vertical",marginBottom:14,transition:"border-color 0.2s"}}
                onFocus={e=>e.target.style.borderColor=T.amber}
                onBlur={e=>e.target.style.borderColor=T.border}/>
              ...
              <div style={{display:"flex",gap:8}}>
                <button onClick={async()=>{
                  ...
                  if(!nlForm.title.trim()||!nlForm.body.trim()){setNlError("Title and body required");return;}
                  try{
                    if(nlEditing){
                      await API.patch(`newsletter/manage/${nlEditing}/`,nlForm);
                      setNlSuccess("Post updated!");
                    } else {
                      await API.post("newsletter/manage/",nlForm);
                      setNlSuccess("Post published!");
                    }
                    setNlForm({title:"",body:"",category:"general",emoji:"✂️",pinned:false});
                    setNlEditing(null);
                    loadNewsletter();
                  }catch(e){setNlError(e.response?.data?.error||"Could not save.");}
                }} style={{padding:"12px 24px",background:nlEditing?"#635bff":T.amber,color:"black",...sf,fontSize:7.5,fontWeight:700,letterSpacing:"0.2em",textTransform:"uppercase",border:"none",cursor:"pointer",transition:"opacity 0.2s",
                  clipPath:"polygon(0 0,calc(100% - 7px) 0,100% 7px,100% 100%,7px 100%,0 calc(100% - 7px))"
                }}
                  onMouseEnter={e=>e.currentTarget.style.opacity="0.85"}
                  onMouseLeave={e=>e.currentTarget.style.opacity="1"}>
                ...
                {nlEditing&&(
                  <button onClick={()=>{setNlEditing(null);setNlForm({title:"",body:"",category:"general",emoji:"✂️",pinned:false});setNlError("");setNlSuccess("");}}
                    style={{padding:"12px 18px",background:"transparent",color:"#71717a",...sf,fontSize:7,letterSpacing:"0.15em",textTransform:"uppercase",border:`1px solid ${T.border}`,cursor:"pointer"}}>
                    Cancel
                  </button>
                )}
              </div>
            ...
            {nlLoading?(
              ...
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                {nlPosts.map(post=>(
                  <div key={post.id} style={{background:T.surface,border:`1px solid ${post.pinned?T.amberBorder:T.border}`,overflow:"hidden",
                    clipPath:"polygon(0 0,calc(100% - 10px) 0,100% 10px,100% 100%,10px 100%,0 calc(100% - 10px))"
                  }}>
                    <div style={{height:2,background:post.pinned?"linear-gradient(to right,#ef4444,#f59e0b)":"rgba(255,255,255,0.05)"}}/>
                    <div style={{padding:"16px 20px"}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10,marginBottom:8,flexWrap:"wrap"}}>
                        ...
                        <div style={{display:"flex",gap:6,flexShrink:0}}>
                          <button onClick={()=>{setNlEditing(post.id);setNlForm({title:post.title,body:post.body,category:post.category,emoji:post.emoji,pinned:post.pinned});setNlError("");setNlSuccess("");window.scrollTo({top:0,behavior:"smooth"});}}
                            style={{padding:"6px 14px",...sf,fontSize:6,letterSpacing:"0.15em",textTransform:"uppercase",background:"transparent",border:`1px solid ${T.border}`,color:"#a1a1aa",cursor:"pointer",transition:"all 0.2s"}}
                            onMouseEnter={e=>{e.currentTarget.style.borderColor=T.amber;e.currentTarget.style.color=T.amber;}}
                            onMouseLeave={e=>{e.currentTarget.style.borderColor=T.border;e.currentTarget.style.color="#a1a1aa";}}>
                            Edit
                          </button>
                          <button onClick={async()=>{
                      ...
                      <p style={{...mono,fontSize:12,color:"#a1a1aa",lineHeight:1.75,whiteSpace:"pre-wrap"}}>{post.body}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {activeTab==="pricing"&&(
          <div className="bd-enter" style={{maxWidth:580}}>
            ...
            {/* Header */}
            <div style={{marginBottom:28}}>
              <p style={{...mono,fontSize:8,color:"rgba(245,158,11,0.5)",letterSpacing:"0.5em",textTransform:"uppercase",marginBottom:6}}>HEADZ UP · YOUR RATES</p>
              <h2 style={{...sf,fontSize:22,fontWeight:900,textTransform:"uppercase",letterSpacing:"-0.03em",margin:0}}>
                Service<br/><span style={{color:T.amber,fontStyle:"italic"}}>Pricing_</span>
              </h2>
              <p style={{...mono,fontSize:12,color:T.muted,marginTop:10,lineHeight:1.7}}>
                Set your own prices for each service. Your custom price overrides the shop default. Clients will see your rate when booking with you.
              </p>
            </div>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {pricingList.map(svc=>{
                  ...
                  const savePrice = async()=>{
                    ...
                    try{
                      await API.post("barber/service-prices/",{service_id:svc.id,price:val});
                      setPricingList(p=>p.map(s=>s.id===svc.id?{...s,custom_price:val,effective_price:val,is_custom:true}:s));
                      setPricingEdits(p=>{ const n={...p}; delete n[svc.id]; return n; });
                      showToast(`✓ ${svc.name} updated to $${val.toFixed(2)}`);
                    }catch(e){ showToast(e.response?.data?.error||"Could not update price.","error"); }
                    finally{ setPricingSaving(p=>({...p,[svc.id]:false})); }
                  };
                  ...
                  const resetPrice = async()=>{
                    ...
                    try{
                      ...
                      setPricingList(p=>p.map(s=>s.id===svc.id?{...s,custom_price:null,effective_price:s.default_price,is_custom:false}:s));
                      setPricingEdits(p=>{ const n={...p}; delete n[svc.id]; return n; });
                      showToast(`${svc.name} reset to shop default ($${svc.default_price.toFixed(2)})`);
                    }catch(e){ showToast("Could not reset.","error"); }
                    finally{ setPricingSaving(p=>({...p,[svc.id]:false})); }
                  };

                  return(
                    <div key={svc.id} style={{background:T.surface,border:`1px solid ${hasCustom?"rgba(245,158,11,0.2)":T.border}`,
                      clipPath:"polygon(0 0,calc(100% - 8px) 0,100% 8px,100% 100%,8px 100%,0 calc(100% - 8px))",
                      ...
                      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
                        ...
                        <div style={{flex:1,minWidth:160}}>
                          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
                            ...
                            {hasCustom&&(
                              <span style={{...mono,fontSize:7,color:T.amber,padding:"2px 7px",background:"rgba(245,158,11,0.08)",border:"1px solid rgba(245,158,11,0.2)"}}>
                                CUSTOM
                              </span>
                            )}
                          </div>
                        ...
                        <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
                          {editing ? (
                            <>
                              <div style={{display:"flex",alignItems:"center",background:"#050505",border:`1px solid ${T.amber}`,overflow:"hidden"}}>
                                <span style={{...mono,fontSize:14,color:T.amber,padding:"0 8px 0 12px",lineHeight:1}}>$</span>
                                <input
                                  type="number" min="0" step="0.50"
                                  value={editVal}
                                  onChange={e=>setPricingEdits(p=>({...p,[svc.id]:e.target.value}))}
                                  onKeyDown={e=>{if(e.key==="Enter")savePrice();if(e.key==="Escape")setPricingEdits(p=>{const n={...p};delete n[svc.id];return n;});}}
                                  autoFocus
                                  style={{width:80,padding:"10px 10px 10px 0",background:"transparent",border:"none",color:"white",...mono,fontSize:14,outline:"none"}}
                                />
                              </div>
                              <button onClick={savePrice} disabled={saving}
                                style={{padding:"10px 14px",background:saving?"#111":T.amber,color:saving?T.dim:"black",...sf,fontSize:7,fontWeight:700,letterSpacing:"0.15em",textTransform:"uppercase",border:"none",cursor:saving?"not-allowed":"pointer",transition:"all 0.2s"}}>
                                {saving?"...":"Save"}
                              </button>
                ...
                {/* Footer note */}
                <div style={{marginTop:8,padding:"12px 14px",background:"rgba(245,158,11,0.04)",border:"1px solid rgba(245,158,11,0.1)"}}>
                  <p style={{...mono,fontSize:10,color:"rgba(245,158,11,0.5)",lineHeight:1.7,margin:0}}>
        ...
        {activeTab==="availability"&&(
          <div className="bd-enter" style={{maxWidth:640}}>
            ...
            {/* ── PROFILE PHOTO ── */}
            <div style={{marginBottom:28,background:T.surface,border:`1px solid rgba(245,158,11,0.25)`,
              clipPath:"polygon(0 0,calc(100% - 10px) 0,100% 10px,100% 100%,10px 100%,0 calc(100% - 10px))"
            }}>
              {/* Header */}
              <div style={{background:"#000",padding:"14px 20px",borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",gap:10}}>
                <div style={{width:3,height:22,background:T.amber,flexShrink:0}}/>
                <div>
                  <p style={{...mono,fontSize:7,color:"rgba(245,158,11,0.5)",letterSpacing:"0.5em",textTransform:"uppercase",marginBottom:1}}>Profile</p>
                  <p style={{...sf,fontSize:11,fontWeight:900,textTransform:"uppercase",margin:0}}>Your Photo</p>
                </div>
              </div>

              <div style={{padding:"24px 20px",display:"flex",gap:24,alignItems:"flex-start",flexWrap:"wrap"}}>
                ...
                <div style={{position:"relative",flexShrink:0}}>
                  <div style={{
                    ...
                    boxShadow: photoPreview||barber?.photo_url ? "0 0 24px rgba(245,158,11,0.2)" : "none",
                  }}>
                    {uploadingPhoto ? (
                      <div style={{width:"100%",height:"100%",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:8}}>
                        <div style={{width:22,height:22,border:"2px solid rgba(245,158,11,0.2)",borderTopColor:"#f59e0b",borderRadius:"50%",animation:"spin 0.8s linear infinite"}}/>
                        <p style={{...mono,fontSize:8,color:"rgba(245,158,11,0.6)",letterSpacing:"0.3em"}}>SAVING</p>
                      </div>
                  ...
                  {(photoPreview||barber?.photo_url)&&!uploadingPhoto&&(
                    <div style={{position:"absolute",bottom:4,right:4,width:12,height:12,borderRadius:"50%",background:"#4ade80",border:"2px solid #000",boxShadow:"0 0 6px rgba(74,222,128,0.8)"}}/>
                  )}
                </div>
                ...
                <div style={{flex:1,minWidth:200}}>
                  ...
                  <label style={{display:"block",cursor:uploadingPhoto?"not-allowed":"pointer"}}>
                    <input
                      type="file"
                      accept="image/*"
                      style={{display:"none"}}
                      ...
                      onChange={async(e)=>{
                        ...
                        if(file.size > 5*1024*1024){
                          showToast("Image must be under 5MB — try a smaller photo.","error");
                          return;
                        }
                        ...
                        reader.onload = async(ev)=>{
                          ...
                          try{
                            await API.patch("barber/me/update/",{photo:b64});
                            ...
                            showToast(err.response?.data?.error||"Upload failed — try a smaller image.","error");
                          }finally{
                    ...
                    <div style={{
                      ...
                      padding:"16px 20px",
                      background:uploadingPhoto?"#0a0a0a":"rgba(245,158,11,0.1)",
                      border:`2px solid ${uploadingPhoto?"rgba(245,158,11,0.2)":"rgba(245,158,11,0.6)"}`,
                      cursor:uploadingPhoto?"not-allowed":"pointer",
                      transition:"all 0.2s",
                      clipPath:"polygon(0 0,calc(100% - 8px) 0,100% 8px,100% 100%,8px 100%,0 calc(100% - 8px))",
                    }}
                      onMouseEnter={e=>{if(!uploadingPhoto){e.currentTarget.style.background="rgba(245,158,11,0.18)";e.currentTarget.style.borderColor="#f59e0b";}}}
                      onMouseLeave={e=>{if(!uploadingPhoto){e.currentTarget.style.background="rgba(245,158,11,0.1)";e.currentTarget.style.borderColor="rgba(245,158,11,0.6)";}}}
                    >
                      <span style={{fontSize:20}}>{uploadingPhoto?"⏳":"📸"}</span>
                      <div>
                        <p style={{...sf,fontSize:9,fontWeight:700,letterSpacing:"0.15em",textTransform:"uppercase",color:uploadingPhoto?"#52525b":"#f59e0b",margin:0}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:16}}>
              <div style={{width:3,height:28,background:"#635bff",flexShrink:0}}/>
              <div>
                <p style={{...mono,fontSize:7,color:"rgba(99,91,255,0.6)",letterSpacing:"0.5em",textTransform:"uppercase",marginBottom:2}}>Online Payments</p>
                <p style={{...sf,fontSize:13,fontWeight:900,textTransform:"uppercase",letterSpacing:"0.02em"}}>Stripe Connect</p>
              </div>
            </div>
            <StripeConnectPanel barber={barber} isMobile={isMobile}/>

            {/* ── Cash App fallback ── */}
            <div style={{marginBottom:28,padding:"16px 18px",background:"rgba(0,212,95,0.04)",border:"1px solid rgba(0,212,95,0.12)"}}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
                <div style={{width:28,height:28,background:"#00d45f",borderRadius:6,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                  <span style={{...sf,fontSize:12,fontWeight:900,color:"black"}}>$</span>
                </div>
                <div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:8}}>
              ...
              <p style={{...mono,fontSize:8,color:T.amber,letterSpacing:"0.5em",textTransform:"uppercase"}}>My Hours</p>
            </div>
            <p style={{...mono,fontSize:12,color:T.muted,lineHeight:1.7,marginBottom:24}}>Set your working hours for each day. Clients can only book during these times.</p>
            <div style={{display:"flex",flexDirection:"column",gap:6}}>
              {DAYS.map((dayName,dayIdx)=>{
                const saved = availability.find(a=>a.day_of_week===dayIdx);
                const isEditing = editingDay===dayIdx;
                const isWorking = saved?.is_working??(dayIdx<6);
                const isSun = dayIdx===6;
                return(
                  <div key={dayIdx} style={{background:T.surface,border:`1px solid ${isEditing?T.amberBorder:T.border}`,overflow:"hidden",transition:"all 0.2s"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 18px",flexWrap:"wrap",gap:10}}>
                      ...
                      {!isSun&&(
                        <button onClick={()=>{
                          ...
                          setEditWorking(saved?saved.is_working:dayIdx<6);
                          setEditStart(saved?.start_time?.slice(0,5)||"09:00");
                          setEditEnd(saved?.end_time?.slice(0,5)||"18:00");
                        }}
                          style={{...sf,fontSize:6,letterSpacing:"0.15em",textTransform:"uppercase",padding:"6px 14px",background:isEditing?T.amberDim:"transparent",border:`1px solid ${isEditing?T.amber:T.border}`,color:isEditing?T.amber:T.muted,cursor:"pointer",transition:"all 0.2s"}}>
                          {isEditing?"✕ Cancel":"Edit"}
                        </button>
                      )}
                    </div>
                    {isEditing&&(
                      <div style={{padding:"16px 18px",borderTop:`1px solid ${T.border}`,background:T.amberDim}}>
                        <div style={{display:"flex",gap:6,marginBottom:14}}>
                          {[{v:true,l:"Working"},{v:false,l:"Day Off"}].map(({v,l})=>(
                            <button key={l} onClick={()=>setEditWorking(v)}
                              style={{flex:1,padding:"9px",...sf,fontSize:7,textTransform:"uppercase",letterSpacing:"0.15em",background:editWorking===v?T.amber:"transparent",color:editWorking===v?"black":T.muted,border:`1px solid ${editWorking===v?T.amber:T.border}`,cursor:"pointer",transition:"all 0.2s"}}>
                              {l}
                            </button>
                          ))}
                        </div>
                        {editWorking&&(
                          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
                            {[{l:"Start",v:editStart,s:setEditStart},{l:"End",v:editEnd,s:setEditEnd}].map(({l,v,s})=>(
                              <div key={l}>
                                <label style={{...sf,fontSize:6,letterSpacing:"0.3em",color:T.muted,textTransform:"uppercase",display:"block",marginBottom:6}}>{l}</label>
                                <input type="time" value={v} onChange={e=>s(e.target.value)}
                                  style={{width:"100%",background:T.bg,border:`1px solid ${T.border}`,padding:"10px 12px",color:"white",fontSize:14,outline:"none",...mono}}
                                  onFocus={e=>e.target.style.borderColor=T.amber}
                                  onBlur={e=>e.target.style.borderColor=T.border}/>
                              </div>
                            ))}
                        ...
                        <button onClick={saveAvailability} disabled={savingAvail}
                          style={{padding:"11px 24px",background:savingAvail?T.deep:T.amber,color:savingAvail?T.dim:"black",...sf,fontSize:7,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.2em",border:"none",cursor:savingAvail?"not-allowed":"pointer",transition:"all 0.2s",display:"flex",alignItems:"center",gap:8}}>
                          {savingAvail?<><span style={{width:10,height:10,border:"2px solid #3f3f46",borderTopColor:"#71717a",borderRadius:"50%",display:"inline-block",animation:"spin 0.7s linear infinite"}}/>Saving...</>:"Save Hours →"}
                        </button>
                      </div>
                    )}
                  </div>
        ...
        {activeTab==="timeoff"&&(
          <div className="bd-enter" style={{maxWidth:600}}>
            <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:8}}>
              <div style={{width:32,height:1,background:`rgba(245,158,11,0.5)`}}/>
              <p style={{...mono,fontSize:8,color:T.amber,letterSpacing:"0.5em",textTransform:"uppercase"}}>Time Off</p>
            </div>
            <p style={{...mono,fontSize:12,color:T.muted,lineHeight:1.7,marginBottom:24}}>Block specific dates. Clients won't be able to book you on these days.</p>
            <div style={{padding:"20px",background:T.surface,border:`1px solid ${T.border}`,marginBottom:20}}>
              <p style={{...sf,fontSize:7,letterSpacing:"0.3em",color:T.muted,textTransform:"uppercase",marginBottom:16}}>Block a Date</p>
              <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 2fr",gap:12,marginBottom:14}}>
                {[
                  {label:"Date",type:"date",val:newTimeOffDate,set:setNewTimeOffDate,min:today},
                  {label:"Reason (optional)",type:"text",val:newTimeOffReason,set:setNewTimeOffReason,placeholder:"Vacation, personal, etc."},
                ].map(({label,type,val,set,min,placeholder})=>(
                  <div key={label}>
                    <label style={{...sf,fontSize:6,letterSpacing:"0.3em",color:T.muted,textTransform:"uppercase",display:"block",marginBottom:8}}>{label}</label>
                    <input type={type} value={val} min={min} onChange={e=>set(e.target.value)} placeholder={placeholder}
                      style={{width:"100%",background:T.bg,border:`1px solid ${T.border}`,padding:"11px 12px",color:"white",fontSize:type==="date"?14:16,outline:"none",...mono}}
                      onFocus={e=>e.target.style.borderColor=T.amber}
                      onBlur={e=>e.target.style.borderColor=T.border}/>
                  </div>
                ))}
              </div>
              <button onClick={addTimeOff} disabled={!newTimeOffDate||addingTimeOff}
                style={{padding:"11px 24px",background:!newTimeOffDate||addingTimeOff?T.deep:T.amber,color:!newTimeOffDate||addingTimeOff?T.dim:"black",...sf,fontSize:7,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.2em",border:"none",cursor:!newTimeOffDate||addingTimeOff?"not-allowed":"pointer",transition:"all 0.25s"}}>
