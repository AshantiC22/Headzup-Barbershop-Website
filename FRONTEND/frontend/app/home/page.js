...
/* ═══════════════════════════════════════════════════════════════════════════
   PERSONA SELECT  —  Def Jam / P5 fighter-select
═══════════════════════════════════════════════════════════════════════════ */
    ...
    <section id="barber" style={{ position:"relative", overflow:"hidden", background:"#000", borderTop:`3px solid ${A}`, borderBottom:`3px solid ${A}` }}>
      ...
      <div style={{ position:"absolute", inset:0, zIndex:0 }}>
        {validPhoto(active.photo_url||active.photo) && (
          <img key={"bg-"+active.id} src={validPhoto(active.photo_url||active.photo)} alt="" style={{ width:"100%", height:"100%", objectFit:"cover", objectPosition:"center top", filter:"brightness(0.08) saturate(0.2)" }}/>
        )}
      ...
      <div style={{ position:"relative", zIndex:2, maxWidth:1320, margin:"0 auto", padding:isMobile?"44px 20px 52px":"0 44px", minHeight:isMobile?"auto":"100vh", display:"flex", flexDirection:"column", justifyContent:"center" }}>
        ...
        {isMobile ? (
          <PSMobile list={list} sel={sel} setSel={setSel} active={active} locked={locked} setLocked={setLocked} flash={flash} lock={lock} STATS={STATS} book={book}/>
        ) : (<PSDesktop list={list} sel={sel} setSel={setSel} active={active} locked={locked} setLocked={setLocked} flash={flash} lock={lock} STATS={STATS} book={book}/>
        )}
...
function PSMobile({ list, sel, setSel, active, locked, setLocked, flash, lock, STATS, book }) {
  return (
    <div>
          ...
          <button key={b.id} onClick={() => { setSel(i); setLocked(false); }}
            ...
            {validPhoto(b.photo_url||b.photo) ? (
              <img src={validPhoto(b.photo_url||b.photo)} alt={b.name} style={{ width:"100%", height:"100%", objectFit:"cover", objectPosition:"center top" }}/>
                ...
                <span style={{ fontFamily:"'Syncopate',sans-serif", fontSize:28, color:A, fontWeight:900 }}>{(b.name||"?").charAt(0)}</span>
              </div>
            )}
      ...
      <div className={flash?"ps-flash":""}>
        <div style={{ display:"inline-flex", background:locked?"#22c55e":A, padding:"4px 12px 4px 10px", clipPath:"polygon(0 0,calc(100% - 6px) 0,100% 6px,100% 100%,6px 100%,0 calc(100% - 6px))", marginBottom:12 }}>
          <span style={{ fontFamily:"'DM Mono',monospace", fontSize:8, color:"#000", letterSpacing:"0.4em", textTransform:"uppercase" }}>{locked?"✓ LOCKED IN":"BARBER PROFILE"}</span>
        </div>
        <h2 key={"m-"+active.id} className="ps-name" style={{ fontFamily:"'Syncopate',sans-serif", fontSize:"clamp(2rem,8vw,2.8rem)", fontWeight:900, letterSpacing:"-0.04em", lineHeight:1.05, marginBottom:10, color:locked?A:"white", textTransform:"uppercase" }}>{active.name}</h2>
        {active.bio && <p style={{ fontFamily:"'DM Mono',monospace", fontSize:12, color:"#a1a1aa", lineHeight:1.7, marginBottom:16 }}>{active.bio}</p>}
        <div style={{ display:"flex", flexDirection:"column", gap:8, marginBottom:20 }}>
          {STATS.map(s => (
            <div key={s.k} style={{ display:"flex", alignItems:"center", gap:10 }}>
              <span style={{ fontFamily:"'DM Mono',monospace", fontSize:7, color:"#a1a1aa", letterSpacing:"0.3em", textTransform:"uppercase", width:64, flexShrink:0 }}>{s.k}</span>
              ...
              <span style={{ fontFamily:"'DM Mono',monospace", fontSize:9, color:A, minWidth:24, textAlign:"right" }}>{s.v}</span>
            </div>
          ))}
        </div>
      </div>
      {locked ? (
        <div style={{ display:"flex", gap:8 }}>
          <button onClick={lock} style={{ flex:1, padding:"16px", background:"#22c55e", color:"black", fontFamily:"'Syncopate',sans-serif", fontSize:8, fontWeight:700, letterSpacing:"0.2em", textTransform:"uppercase", border:"none", cursor:"pointer", clipPath:"polygon(0 0,calc(100% - 8px) 0,100% 8px,100% 100%,8px 100%,0 calc(100% - 8px))" }}>✓ CONFIRM</button>
          <button onClick={() => setLocked(false)} style={{ padding:"16px 18px", background:"transparent", color:"#a1a1aa", border:"1px solid rgba(255,255,255,0.1)", cursor:"pointer", fontFamily:"'DM Mono',monospace", fontSize:10, minHeight:"auto" }}>✕</button>
        </div>
      ) : (<button onClick={lock} style={{ width:"100%", padding:"16px", background:A, color:"black", fontFamily:"'Syncopate',sans-serif", fontSize:8, fontWeight:700, letterSpacing:"0.22em", textTransform:"uppercase", border:"none", cursor:"pointer", clipPath:"polygon(0 0,calc(100% - 8px) 0,100% 8px,100% 100%,8px 100%,0 calc(100% - 8px))" }}>Book {active.name} →</button>
      )}
    </div>
  );
}

function PSDesktop({ list, sel, setSel, active, locked, setLocked, flash, lock, STATS, book }) {
  const STATS_ROWS = [{k:"FADE",v:98},{k:"LINEUP",v:99},{k:"BEARD",v:95},{k:"PRECISION",v:97},{k:"VIBE",v:100}];
  return (
    <div style={{ display:"grid", gridTemplateColumns:"360px 1fr", gap:0, alignItems:"stretch" }}>
      ...
      <div style={{ borderRight:`1px solid ${A}22`, paddingRight:32, paddingTop:20, paddingBottom:20 }}>
        {list.map((b,i) => (
          <div key={b.id} onClick={() => { setSel(i); setLocked(false); }} className="ps-row"
            ...
            <div style={{ width:54, height:68, flexShrink:0, overflow:"hidden", position:"relative" }}>
              {validPhoto(b.photo_url||b.photo) ? (
                <img src={validPhoto(b.photo_url||b.photo)} alt={b.name} style={{ width:"100%", height:"100%", objectFit:"cover", objectPosition:"center top", filter:i===sel?"none":"brightness(0.25) saturate(0.2)" }}/>
              ) : (<div style={{ width:"100%", height:"100%", background:"#111", display:"flex", alignItems:"center", justifyContent:"center" }}>
                  <span style={{ fontFamily:"'Syncopate',sans-serif", fontSize:18, color:i===sel?A:"#333", fontWeight:900 }}>{(b.name||"?").charAt(0)}</span>
                </div>
              )}
            </div>
            <div style={{ flex:1, padding:"10px 14px", display:"flex", flexDirection:"column", justifyContent:"center" }}>
              <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:4 }}>
                ...
                <span style={{ fontFamily:"'Syncopate',sans-serif", fontSize:8, fontWeight:900, color:i===sel?"white":"#52525b", textTransform:"uppercase", letterSpacing:"0.05em" }}>{b.name}</span>
              </div>
              <span style={{ fontFamily:"'DM Mono',monospace", fontSize:9, color:i===sel?`${A}88`:"#3f3f46", letterSpacing:"0.2em" }}>HEADZ UP</span>
            </div>
            {i===sel && (
      ...
      {/* Right detail panel */}
      <div style={{ paddingLeft:44, paddingTop:20, paddingBottom:20, display:"flex", flexDirection:"column", gap:24 }}>
        <div className={flash?"ps-flash":""} style={{ display:"flex", gap:32, alignItems:"flex-start" }}>

          {/* Photo card */}
          <div style={{ flexShrink:0 }}>
            <div style={{ width:220, height:280, position:"relative", clipPath:"polygon(0 0,calc(100% - 16px) 0,100% 16px,100% 100%,16px 100%,0 calc(100% - 16px))", border:`2px solid ${A}44`, overflow:"hidden" }}>
              {validPhoto(active.photo_url||active.photo) ? (
                <img key={"dp-"+active.id} src={validPhoto(active.photo_url||active.photo)} alt={active.name} style={{ width:"100%", height:"100%", objectFit:"cover", objectPosition:"center top" }}/>
              ) : (<div style={{ width:"100%", height:"100%", background:"#111", display:"flex", alignItems:"center", justifyContent:"center" }}>
                  <span style={{ fontFamily:"'Syncopate',sans-serif", fontSize:72, color:A, fontWeight:900 }}>{(active.name||"?").charAt(0)}</span>
                </div>
              )}
              <div style={{ position:"absolute", inset:0, background:"linear-gradient(to top,rgba(0,0,0,0.7) 0%,transparent 50%)" }}/>
              <div style={{ position:"absolute", bottom:12, left:14 }}>
                <p style={{ fontFamily:"'Syncopate',sans-serif", fontSize:11, fontWeight:900, color:"white", textTransform:"uppercase" }}>{active.name}</p>
              </div>
            </div>
            <div style={{ marginTop:14, display:"flex", flexDirection:"column", gap:6 }}>
              {STATS_ROWS.map(s => (
                <div key={s.k} style={{ display:"flex", alignItems:"center", gap:8 }}>
                  <span style={{ fontFamily:"'DM Mono',monospace", fontSize:7, color:"#52525b", letterSpacing:"0.25em", width:58, flexShrink:0 }}>{s.k}</span>
                  <div style={{ flex:1, height:2, background:"rgba(255,255,255,0.05)", position:"relative", overflow:"hidden" }}>
                    <div className="ps-bar" key={"ds-"+active.id+"-"+s.k} style={{ position:"absolute", inset:"0 auto 0 0", background:`linear-gradient(to right,${A},#fbbf24)`, "--w":`${s.v}%`, width:`${s.v}%` }}/>
                  </div>
                  <span style={{ fontFamily:"'DM Mono',monospace", fontSize:9, color:A, minWidth:22, textAlign:"right" }}>{s.v}</span>
                </div>
              ))}
            </div>
            <div style={{ marginTop:14, paddingTop:12, borderTop:"1px solid rgba(255,255,255,0.05)", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div style={{ display:"flex", gap:2 }}>
                {[0,1,2,3,4].map(i => <span key={i} style={{ color:A, fontSize:10 }}>★</span>)}
              </div>
              <span style={{ fontFamily:"'DM Mono',monospace", fontSize:9, color:"#52525b" }}>5.0 · Hattiesburg</span>
            </div>
          </div>
          ...
          <div style={{ flex:1, display:"flex", flexDirection:"column", gap:16 }}>
            <div>
              <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:12 }}>
                ...
                <span style={{ fontFamily:"'DM Mono',monospace", fontSize:7, color:`${A}66`, letterSpacing:"0.5em", textTransform:"uppercase" }}>PROFILE_DATA</span>
              </div>
              {[["SHOP","HEADZ UP"],["CITY","Hattiesburg, MS"],["STYLE","Precision Cuts"],["RATING","★★★★★"],["STATUS","● ONLINE"]].map(([l,v]) => (
                <div key={l} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", paddingBottom:8, marginBottom:8, borderBottom:"1px solid rgba(255,255,255,0.04)" }}>
                  <span style={{ fontFamily:"'DM Mono',monospace", fontSize:7, color:"#52525b", letterSpacing:"0.25em", textTransform:"uppercase" }}>{l}</span>
                  <span style={{ fontFamily:"'DM Mono',monospace", fontSize:10, color:v.startsWith("●")?"#4ade80":v.startsWith("★")?A:"#a1a1aa" }}>{v}</span>
                </div>
              ))}
            </div>
            <div>
              <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:10 }}>
                ...
                <span style={{ fontFamily:"'DM Mono',monospace", fontSize:7, color:`${R}88`, letterSpacing:"0.5em", textTransform:"uppercase" }}>SPECIALTIES</span>
              </div>
              <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                {["Fades","Lineups","Beards","Kids","Locs","Designs"].map(tag => (
                  <span key={tag} style={{ fontFamily:"'DM Mono',monospace", fontSize:8, color:A, padding:"3px 10px", background:`${A}10`, border:`1px solid ${A}25`, clipPath:"polygon(0 0,calc(100% - 4px) 0,100% 4px,100% 100%,4px 100%,0 calc(100% - 4px))" }}>{tag}</span>
                ))}
              </div>
            </div>
            <div style={{ padding:"12px 14px", background:"rgba(245,158,11,0.05)", border:`1px solid ${A}20`, clipPath:"polygon(0 0,calc(100% - 8px) 0,100% 8px,100% 100%,8px 100%,0 calc(100% - 8px))" }}>
              <p style={{ fontFamily:"'DM Mono',monospace", fontSize:9, color:`${A}88`, letterSpacing:"0.3em", textTransform:"uppercase", marginBottom:4 }}>DEPOSIT</p>
              <p style={{ fontFamily:"'Syncopate',sans-serif", fontSize:20, color:A, fontWeight:900 }}>$10 <span style={{ fontFamily:"'DM Mono',monospace", fontSize:9, color:"#52525b" }}>to secure your spot</span></p>
            </div>
            <div style={{ display:"flex", flexDirection:"column", gap:8, marginTop:"auto" }}>
              {locked ? (
                <>
                  <button onClick={lock} style={{ width:"100%", padding:"16px 20px", background:"#22c55e", color:"black", fontFamily:"'Syncopate',sans-serif", fontSize:9, fontWeight:700, letterSpacing:"0.2em", textTransform:"uppercase", border:"none", cursor:"pointer", clipPath:"polygon(0 0,calc(100% - 10px) 0,100% 10px,100% 100%,10px 100%,0 calc(100% - 10px))" }}>✓ LOCKED IN — BOOK NOW</button>
                  <button onClick={() => setLocked(false)} style={{ width:"100%", padding:"13px 20px", background:"transparent", color:"#52525b", border:"1px solid rgba(255,255,255,0.08)", cursor:"pointer", fontFamily:"'Syncopate',sans-serif", fontSize:8, letterSpacing:"0.15em", textTransform:"uppercase", transition:"all 0.2s" }}
                  ...
                  <button onClick={lock} style={{ width:"100%", padding:"16px 20px", background:A, color:"black", fontFamily:"'Syncopate',sans-serif", fontSize:9, fontWeight:700, letterSpacing:"0.2em", textTransform:"uppercase", border:"none", cursor:"pointer", clipPath:"polygon(0 0,calc(100% - 10px) 0,100% 10px,100% 100%,10px 100%,0 calc(100% - 10px))", transition:"background 0.2s" }}
                    onMouseEnter={e => e.currentTarget.style.background="white"}
                    onMouseLeave={e => e.currentTarget.style.background=A}>
                    BOOK {active.name.split(" ")[0].toUpperCase()} →
                  </button>
                  <a href="/book" onClick={book} style={{ width:"100%", padding:"13px 20px", background:"transparent", color:"#52525b", fontFamily:"'Syncopate',sans-serif", fontSize:8, letterSpacing:"0.2em", textTransform:"uppercase", border:"1px solid rgba(255,255,255,0.08)", cursor:"pointer", textDecoration:"none", display:"block", textAlign:"center", transition:"all 0.2s" }}
      ...
      <div style={{ position:"relative",zIndex:10 }}>
        ...
        <nav style={{ position:"fixed",top:0,left:0,right:0,zIndex:200,height:56,background:scrollY>50?"rgba(3,3,3,0.97)":"transparent",backdropFilter:scrollY>50?"blur(20px)":"none",borderBottom:scrollY>50?`1px solid rgba(255,255,255,0.06)`:"none",transition:"all 0.4s",display:"flex",alignItems:"center" }}>
          <div style={{ width:"100%",maxWidth:1320,margin:"0 auto",padding:"0 clamp(18px,5vw,44px)",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
            <a href="/" style={{ textDecoration:"none", display:"flex", alignItems:"center" }}>
              <img src="/logo1.jpg" alt="Headz Up Barbershop" style={{ height:38, width:"auto", objectFit:"contain" }}/>
            </a>
        ...
        <section style={{ minHeight:"100dvh",position:"relative",overflow:"hidden",display:"flex",alignItems:"flex-end" }}>
          ...
          <div style={{ position:"relative",zIndex:1,maxWidth:1320,margin:"0 auto",width:"100%",padding:isMobile?"0 20px 56px":"0 clamp(18px,5vw,44px) 80px",paddingTop:isMobile?78:118 }}>
            ...
            {/* Logo — styled to fit dark P5/DMC aesthetic */}
            <div style={{
              ...
              <div style={{
                ...
                />
                {/* Bottom fade so it blends into dark bg */}
                <div style={{
                  position:"absolute",
